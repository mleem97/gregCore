...
## 2025-02-14 - Optimize Hardware Lookups
**Learning:** Found an O(N) Unity scene query `FindObjectsOfType<Server>()` being used for hardware iteration, despite `NetworkMap.instance` providing O(1) dictionary lookups (`.servers`, `.switches`, `.patchPanels`).
**Action:** Replaced `FindObjectsOfType` calls in `GameHooks.cs` and `LuaServerModule.cs` with dictionary lookups via `NetworkMap.instance` with fallbacks, ensuring O(1) iteration complexity for frequent queries without sacrificing reliability.
