...
**Action:** Replace `FindObjectsOfType<Il2Cpp.Rack>()` calls with lookups to `NetworkMap.instance.GetNumberOfDevices()[2]`, preserving fallback behavior for when the map instance or arrays are not yet initialized, maintaining correctness while bypassing expensive Unity hierarchy scanning.
