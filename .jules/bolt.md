...
## 2023-10-27 - [Optimize Rack Count Calls]
**Learning:** `UnityEngine.Object.FindObjectsOfType<T>` is an expensive O(N) operation in Unity/IL2CPP, crossing the managed-native boundary and allocating memory each call. Caching the instances via Harmony patches on `Awake` and `OnDestroy` avoids this, but requires careful handling of IL2CPP dead wrappers (checking `Pointer != IntPtr.Zero`) and checking `activeInHierarchy` to match the original behavior of `FindObjectsOfType`. In C# 10, file-scoped namespaces (e.g. `namespace gregCore.GameLayer.Patches.Hardware;`) are valid but can confuse automated reviewers into thinking a class with no indentation is in the global namespace.
**Action:** When caching objects using Harmony patches, always check for dead native pointers (`Pointer != IntPtr.Zero`), clear the cache on scene transitions to prevent leaks, filter by `activeInHierarchy` when replacing `FindObjectsOfType`, and use thread-safe locks (`_cacheLock`) to avoid concurrent modifications. Ignore false positive review flags regarding file-scoped namespaces if `dotnet build` succeeds.
