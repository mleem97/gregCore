...
## 2024-05-19 - Cache Racks to avoid FindObjectsOfType
**Learning:** `UnityEngine.Object.FindObjectsOfType` is O(N) and can be extremely expensive, especially when called repeatedly from Lua scripts or API methods.
**Action:** Replace `FindObjectsOfType<Il2Cpp.Rack>()` with a cached `HashSet<global::Il2Cpp.Rack>` updated via Harmony patches on `Awake` and `OnDestroy`. Clear the cache in `OnSceneWasLoaded` for `MainMenu` to prevent memory leaks and ghost objects.
