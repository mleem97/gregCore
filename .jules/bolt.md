...
## 2025-05-21 - Expensive Array Scanning for Element Counts
**Learning:** Checking for element counts using `UnityEngine.Object.FindObjectsOfType<T>().Length` in Unity is extremely inefficient because it performs a full scene object scan and allocates arrays. In `gregCore`, the game's `Il2Cpp.NetworkMap.instance` maintains internal indexed caches of these counts.
**Action:** When querying object counts for servers, switches, or racks, avoid `FindObjectsOfType`. Instead, use `Il2Cpp.NetworkMap.instance.GetNumberOfDevices()` which returns an array where index 0 = servers, index 1 = switches, and index 2 = racks. Always perform null-checks on `instance`, the returned array, and bound-checks on the index, falling back to `FindObjectsOfType<T>().Length` if necessary.
