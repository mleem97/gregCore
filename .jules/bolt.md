...
## 2025-06-05 - Expensive Polling for Rack Count
**Learning:** `UnityEngine.Object.FindObjectsOfType<Il2Cpp.Rack>()` was being repeatedly used to query the number of racks in the game. Unlike servers or switches, `Il2Cpp.NetworkMap.instance` does not expose a `.racks` list property. Calling `FindObjectsOfType` scans the entire object hierarchy, which is very slow.
**Action:** To reliably retrieve the count of racks (and other devices) without O(N) hierarchy traversal, use `NetworkMap.instance.GetNumberOfDevices()` which returns an integer array (index 0 = servers, 1 = switches, 2 = racks).
