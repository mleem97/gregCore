...
## 2025-05-21 - Caching Rack instances instead of FindObjectsOfType
**Learning:** `FindObjectsOfType` is an O(N) operation over all loaded objects. Repeatedly polling `UnityEngine.Object.FindObjectsOfType<Il2Cpp.Rack>()` to retrieve racks in APIs causes significant hitches and performance drops.
**Action:** Used `HashSet<global::Il2Cpp.Rack>` in `RackPatch.cs` to cache `Rack` objects hooked into `Awake` and `OnDestroy`. Safely return only elements where `gameObject.activeInHierarchy == true` to emulate `FindObjectsOfType` filtering.
