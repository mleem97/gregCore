...
## 2025-05-28 - Optimized GetRackCount in Public API
**Learning:** `GregFacilityModule.GetRackCount()` was using `UnityEngine.Object.FindObjectsOfType<global::Il2Cpp.Rack>().Length` which causes expensive O(N) heap traversals every time it is queried by modders.
**Action:** Replaced the O(N) lookup with an O(1) array index access via `Il2Cpp.NetworkMap.instance.GetNumberOfDevices()[DEVICE_INDEX_RACKS]` (index 2 for racks).
