...
## 2024-05-22 - Expensive Polling for Count Check in API
**Learning:** `UnityEngine.Object.FindObjectsOfType<Rack>` was being used in global counts check for racks. Finding objects of a type across the entire hierarchy is very expensive, especially as the number of devices or objects grow over time. NetworkMap tracks the counts of devices natively.
**Action:** Always prefer using global singleton collections managed by the game or dedicated methods over calling `FindObjectsOfType<T>`. For example, use `Il2Cpp.NetworkMap.instance.GetNumberOfDevices()` to get rack count.
