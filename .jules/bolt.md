...
## 2024-05-15 - Fast O(1) Lookup with Il2Cpp singletons
**Learning:** In Unity/Il2Cpp environments, `UnityEngine.Object.FindObjectsOfType<T>()` is notoriously slow as it traverses every single GameObject in the active scene. Modding APIs should never use this in frequently called methods like Lua modules or updates. Furthermore, modifying game state (like repairing a device) can alter singletons (like `brokenServers`), necessitating copying keys before iteration to prevent `InvalidOperationException`.
**Action:** Replace `FindObjectsOfType` with direct access to singletons/cached collections (`Il2Cpp.NetworkMap.instance.servers` and `brokenServers`) and O(1) API counts (`API.GregAPI.GetServerCount()`). Check `Pointer != IntPtr.Zero` to avoid native crashes when interacting with destroyed Unity objects. Always copy collection keys before iterating if the loop body mutates the underlying collection.
