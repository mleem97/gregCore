...
## 2026-05-16 - IL2CPP Object Caching Strategy\n**Learning:** In IL2CPP Unity games, `UnityEngine.Object.FindObjectsOfType<T>()` is exceptionally expensive, especially when invoked frequently via Lua bridging or public APIs. We successfully replaced this for `Il2Cpp.Rack` with a cached approach using `HashSet<T>` maintained via Harmony `Awake` and `OnDestroy` patches.\n**Action:** Use this `HashSet<T>` caching pattern alongside `Awake`/`OnDestroy` hook management for other frequently-queried objects. Remember to filter by `gameObject.activeInHierarchy` and `pointer != IntPtr.Zero` when returning the cached instances to accurately simulate `FindObjectsOfType` behavior without regressions.
