...
## 2025-05-21 - Expensive Object Counting in Rack and Server APIs
**Learning:** `UnityEngine.Object.FindObjectsOfType<Il2Cpp.Rack>` and `Il2Cpp.Server` were still being used for counting devices in `GregAPI`, `GregFacilityModule`, `LuaRackModule` and `LuaServerModule`. Finding objects of a type across the entire hierarchy just to count them is a massive performance bottleneck.
**Action:** Use `Il2Cpp.NetworkMap.instance.GetNumberOfDevices()` (index 2 for racks) and `Il2Cpp.NetworkMap.instance.servers.Count` for O(1) counting instead of searching the entire object hierarchy. Always prefer using global singleton collections managed by the game over calling `FindObjectsOfType<T>`. Ensure null checks are present.
