...
## 2025-05-23 - Optimize FindObjectsOfType calls using Game Singletons
**Learning:** Checking for device counts via `UnityEngine.Object.FindObjectsOfType<Il2Cpp.Rack>()` is an expensive O(N) operation across the entire Unity scene hierarchy. `Il2Cpp.NetworkMap.instance` provides access to global arrays such as `GetNumberOfDevices()` which operates in O(1) time.
**Action:** When performing device counts, try to read them from game-managed singletons first, falling back to `FindObjectsOfType` only if the game objects are not initialized (e.g. during menu load).
