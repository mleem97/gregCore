...
**Action:** When trying to eliminate GC pressure from allocations on collections that will be mutated during iteration, use an early return (`if (collection.Count == 0) return;`) to avoid the allocation when the collection is empty, while keeping the defensive copy when work actually needs to be done.
...
**Learning:** `UnityEngine.Object.FindObjectsOfType<T>` was being used in several global counts check via `GregAPI`, `GregServerModule`, `GregNetworkModule` and `GregNpcModule`. Finding objects of a type across the entire hierarchy is very expensive.
**Action:** Always prefer using global singleton collections managed by the game over calling `FindObjectsOfType<T>`. Ensure null checks are present.

## 2025-06-01 - Harmony Patches IL2CPP Types Verification
**Learning:** Identifying the correct TargetClass and Method is tricky without game dependencies. Using `framework/greg_hooks.json` or `docs/FrameworkAPI.md` is a reliable way to map unconfirmed patches.
**Action:** Always search the docs or json hooks file for `patchTarget` and `name` attributes when mapping an unresolved ILSpy signature.
