...
## 2025-05-25 - [Optimize Global Rack Counting]
**Learning:** `UnityEngine.Object.FindObjectsOfType<global::Il2Cpp.Rack>()` is extremely expensive as an O(N) traversal over all scene objects. In several places across API modules, Lua endpoints, and compatibility hooks, it was used just to fetch the total rack count.
**Action:** Replace `FindObjectsOfType<Rack>()` queries for total counts with `global::Il2Cpp.NetworkMap.instance.GetNumberOfDevices()[2]`. `GetNumberOfDevices()` returns an array with device counts where index 2 specifically represents the number of racks, functioning as an O(1) indexed lookup instead of an O(N) allocation and hierarchy traversal.
