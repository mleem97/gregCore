...
## 2025-05-22 - Optimize Rack Count lookups using NetworkMap
**Learning:** `UnityEngine.Object.FindObjectsOfType<Il2Cpp.Rack>()` is extremely expensive as it traverses the entire Unity object hierarchy. Similar to the network map optimizations for servers and switches, getting rack counts was blocking performance by calling `FindObjectsOfType<T>` on every query. The `GetNumberOfDevices()` method returning an array contains the counts for servers [0], switches [1], and racks [2].
**Action:** Replace `FindObjectsOfType<Il2Cpp.Rack>()` calls with lookups to `NetworkMap.instance.GetNumberOfDevices()[2]`, preserving fallback behavior for when the map instance or arrays are not yet initialized, maintaining correctness while bypassing expensive Unity hierarchy scanning. Also ensure line endings are preserved when replacing code to prevent massive diffs and git blame breakage.
