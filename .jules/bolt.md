...
## 2025-05-21 - Expensive Rack Count Polling
**Learning:** `UnityEngine.Object.FindObjectsOfType<Rack>()` was being used to count racks in several places (`GameHooks.GetRackCount`, `GregAPI.GetRackCount`, `GregFacilityModule.GetRackCount`, and `LuaRackModule.count`). In IL2CPP environments, `FindObjectsOfType` scans the entire scene hierarchy, which is extremely expensive, especially as the number of devices grows.
**Action:** Replaced `FindObjectsOfType<Rack>()` with `NetworkMap.instance.GetNumberOfDevices()[2]` (index 2 corresponds to racks) which is O(1). Added a null/bounds check and kept the original `FindObjectsOfType` as a fallback for main menu or uninitialized states.
