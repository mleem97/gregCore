...
## 2025-05-20 - Expensive NetworkMap Array Length Check
**Learning:** Checking the length of an array returned by `NetworkMap.instance.GetNumberOfDevices()` using magic numbers is fragile and makes code less readable. `FindObjectsOfType<T>` fallback is still useful for uninitialized states (like the main menu).
**Action:** Use a well-named constant like `DEVICE_INDEX_RACKS = 2` to explain index semantics to other developers. Always keep a safe fallback to `FindObjectsOfType` for edge cases where the game manager or array might be null or shorter than expected.
