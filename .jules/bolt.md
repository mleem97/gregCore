...
## 2024-05-31 - [Optimize device count retrieval in IL2CPP]
**Learning:** `Il2Cpp.NetworkMap.instance.GetNumberOfDevices()` returns an integer array where indices represent device counts (0 = servers, 1 = switches, 2 = racks). This allows for O(1) device count retrieval and avoids the O(N) cost of using `FindObjectsOfType<T>()` or hierarchy traversal.
**Action:** Always prefer `GetNumberOfDevices()` with the appropriate index mapped to a descriptive constant when the goal is only to retrieve the device count rather than iterating over instances.
