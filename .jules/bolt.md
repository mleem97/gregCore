...
## 2024-06-23 - Optimizing Rack Counting using NetworkMap Array
**Learning:** `UnityEngine.Object.FindObjectsOfType<Il2Cpp.Rack>()` was being used in several places to get the rack count (`GregAPI.GetRackCount()`, `LuaRackModule`, `GregFacilityModule`, `GameHooks.GetRackCount()`). Finding objects of a type across the entire hierarchy is very expensive. IL2CPP's `NetworkMap.instance.GetNumberOfDevices()` returns an integer array where index 2 represents the number of racks, offering O(1) count retrieval instead.
**Action:** Replace `FindObjectsOfType<T>()` count checks with the `NetworkMap.instance.GetNumberOfDevices()` array when applicable, falling back to the original lookup method when the singleton or array is null. Always check array length to avoid `IndexOutOfRangeException`.
