...
**Prevention:** Validate input strings that form part of a file path before concatenating them. Reject them if they contain directory traversal characters like `..`, `Path.DirectorySeparatorChar`, `Path.AltDirectorySeparatorChar`, or any invalid filename characters (using `Path.GetInvalidFileNameChars()`).
## 2024-09-20 - Path Traversal in Directory.GetFiles Pattern
**Vulnerability:** The `list_files` Lua module function accepted an unsanitized `pattern` string which was passed directly to `Directory.GetFiles` as the search pattern, allowing for path traversal attacks.
**Learning:** The `searchPattern` parameter in `Directory.GetFiles` is vulnerable to path traversal if it contains directory separators or `..`.
**Prevention:** Always validate or sanitize user-provided search patterns. Reject them if they contain traversal characters like `..`, `Path.DirectorySeparatorChar`, or `Path.AltDirectorySeparatorChar`.
