...
## 2024-05-18 - Path Traversal Prefix Bypass in GregIoLuaModule
**Vulnerability:** A path traversal prefix bypass vulnerability existed in `GregIoLuaModule.cs`. The code used `fullPath.StartsWith(dataDirFull)` to check if a resolved path stayed within the sandbox directory. However, if the `dataDirFull` did not end with a directory separator, a path like `/mods/data_secret/file.txt` could bypass the check against `/mods/data`.
**Learning:** Checking for sandbox containment with `String.StartsWith` on file paths is vulnerable if the base path does not have a trailing directory separator.
**Prevention:** Ensure the base path ends with a directory separator (`Path.DirectorySeparatorChar`) before performing a `StartsWith` check to prevent prefix bypass. Also check for exact match equality if the resolved path points to the sandbox root itself.
