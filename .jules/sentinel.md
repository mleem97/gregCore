...
## 2024-05-23 - Path Traversal Bypass in Prefix String Matching
**Vulnerability:** A path traversal vulnerability existed in the Lua sandbox IO module (`GregIoLuaModule.cs`) because it used `String.StartsWith(baseDir)` without ensuring the base directory string ended with a trailing slash. An attacker could access sibling directories with the same prefix (e.g., escaping `/base/data` to access `/base/data-secret` by using the relative path `../data-secret/test.txt`).
**Learning:** Using `StartsWith` for path validation is inherently dangerous if the base path doesn't explicitly terminate with a directory separator, as it can match unintended paths that simply share the same string prefix.
**Prevention:** When validating paths using string prefixes, always ensure the base directory ends with a trailing slash (`Path.DirectorySeparatorChar`). Alternatively, use `Path.GetRelativePath` and check for `..` in the result, or use robust path containment checks.
