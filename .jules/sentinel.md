...
## 2025-02-24 - Fix Path Traversal via Prefix Matching Bypass
**Vulnerability:** A path traversal vulnerability existed in `GregIoLuaModule.cs` where the sandbox validation used `fullPath.StartsWith(dataDirFull)`. This allowed prefix-matching bypasses because a directory like `data-secret` would match a `StartsWith` check against `data`.
**Learning:** When using `String.StartsWith` for directory boundary checks, it is critical to ensure the base directory string includes a trailing directory separator (e.g., `/base/data/` instead of `/base/data`). Without the separator, `String.StartsWith` treats strings merely as text prefixes, leading to sandbox escapes into adjacent directories sharing the same name prefix.
**Prevention:** Always append `Path.DirectorySeparatorChar` to the base path before applying `StartsWith` validation, and provide an explicit fallback for an exact match against the base directory itself (`fullPath.Equals(baseDir)`) if access to the root folder is intended.
