...
## 2024-06-04 - Path Traversal Sandbox Bypass using StartsWith
**Vulnerability:** A sandbox path traversal vulnerability bypass was found where checking path containment with `.StartsWith()` on directory paths allows access to sibling directories with the same prefix (e.g. `/app/data-malicious/` bypasses `.StartsWith("/app/data")`).
**Learning:** Never use raw string prefix matching like `String.StartsWith` to validate directory sandboxing.
**Prevention:** Ensure directory paths end with a directory separator before checking for string containment, or use the centralized helper `gregCore.Infrastructure.Scripting.GregSandboxHelper.IsPathInsideDirectory(fullPath, baseDir)`.
