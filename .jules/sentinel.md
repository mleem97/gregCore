...
## 2024-06-03 - Path Traversal Bypass via Prefix Match
**Vulnerability:** Path traversal bypass in sandbox boundary checks. The code used `String.StartsWith` to check if a resolved file path started with a designated root directory string. Since the root directory string lacked a trailing slash, a sibling directory with a similar prefix (e.g., `/mod/data` vs `/mod/data-secret`) could bypass the check.
**Learning:** Checking path boundaries strictly by string prefix is dangerous. `Path.GetFullPath` handles `..`, but the prefix match fails to ensure the path is actually a descendant of the root directory.
**Prevention:** When validating if a path resides within a root directory, always ensure the root directory string is terminated with a trailing directory separator (e.g., `Path.DirectorySeparatorChar`) before performing a `StartsWith` check, and include an exact match check to permit access to the root directory itself.
