...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.NETWORK.GenerateFinalPath", |payload| {
...
~~~lua
greg.hook.after("greg.NETWORK.GenerateFinalPath", function(ctx)
...
gregBus.on("greg.NETWORK.GenerateFinalPath", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[NETWORK] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.NETWORK.GenerateFinalPath` as the canonical routing key end-to-end.
