...
sidebar_label: greg.NETWORK.GetAllBrokenServers
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.NETWORK.GetAllBrokenServers", |payload| {
...
~~~lua
greg.hook.after("greg.NETWORK.GetAllBrokenServers", function(ctx)
...
gregBus.on("greg.NETWORK.GetAllBrokenServers", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[NETWORK] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.NETWORK.GetAllBrokenServers` as the canonical routing key end-to-end.
