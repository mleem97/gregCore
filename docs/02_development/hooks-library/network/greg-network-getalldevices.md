...
sidebar_label: greg.NETWORK.GetAllDevices
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.NETWORK.GetAllDevices", |payload| {
...
~~~lua
greg.hook.after("greg.NETWORK.GetAllDevices", function(ctx)
...
gregBus.on("greg.NETWORK.GetAllDevices", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[NETWORK] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.NETWORK.GetAllDevices` as the canonical routing key end-to-end.
