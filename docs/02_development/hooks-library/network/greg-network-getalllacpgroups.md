...
sidebar_label: greg.NETWORK.GetAllLACPGroups
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.NETWORK.GetAllLACPGroups", |payload| {
...
~~~lua
greg.hook.after("greg.NETWORK.GetAllLACPGroups", function(ctx)
...
gregBus.on("greg.NETWORK.GetAllLACPGroups", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[NETWORK] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.NETWORK.GetAllLACPGroups` as the canonical routing key end-to-end.
