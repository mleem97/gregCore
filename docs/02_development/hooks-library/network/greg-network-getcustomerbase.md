...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.NETWORK.GetCustomerBase", |payload| {
...
~~~lua
greg.hook.after("greg.NETWORK.GetCustomerBase", function(ctx)
...
gregBus.on("greg.NETWORK.GetCustomerBase", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[NETWORK] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.NETWORK.GetCustomerBase` as the canonical routing key end-to-end.
