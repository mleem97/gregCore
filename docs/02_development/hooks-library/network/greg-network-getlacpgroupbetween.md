...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.NETWORK.GetLACPGroupBetween", |payload| {
...
~~~lua
greg.hook.after("greg.NETWORK.GetLACPGroupBetween", function(ctx)
...
gregBus.on("greg.NETWORK.GetLACPGroupBetween", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[NETWORK] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.NETWORK.GetLACPGroupBetween` as the canonical routing key end-to-end.
