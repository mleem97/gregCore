...
sidebar_label: greg.NETWORK.GetRopeAttachPoint
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.NETWORK.GetRopeAttachPoint", |payload| {
...
~~~lua
greg.hook.after("greg.NETWORK.GetRopeAttachPoint", function(ctx)
...
gregBus.on("greg.NETWORK.GetRopeAttachPoint", (payload) => {
