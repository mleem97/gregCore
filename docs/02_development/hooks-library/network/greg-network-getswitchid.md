...
sidebar_label: greg.NETWORK.GetSwitchId
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.NETWORK.GetSwitchId", |payload| {
...
~~~lua
greg.hook.after("greg.NETWORK.GetSwitchId", function(ctx)
...
gregBus.on("greg.NETWORK.GetSwitchId", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[NETWORK] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.NETWORK.GetSwitchId` as the canonical routing key end-to-end.
