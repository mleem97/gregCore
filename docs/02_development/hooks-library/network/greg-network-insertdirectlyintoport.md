...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.NETWORK.InsertDirectlyIntoPort", |payload| {
...
~~~lua
greg.hook.after("greg.NETWORK.InsertDirectlyIntoPort", function(ctx)
...
gregBus.on("greg.NETWORK.InsertDirectlyIntoPort", (payload) => {
