...
sidebar_label: greg.NETWORK.IsolatedDevicesRemoved
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.NETWORK.IsolatedDevicesRemoved", |payload| {
...
~~~lua
greg.on("greg.NETWORK.IsolatedDevicesRemoved", function(payload)
...
~~~lua
greg.hook.after("greg.NETWORK.IsolatedDevicesRemoved", function(ctx)
...
gregBus.on("greg.NETWORK.IsolatedDevicesRemoved", (payload) => {
