...
sidebar_label: greg.NETWORK.LACPGroupRemoved
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.NETWORK.LACPGroupRemoved", |payload| {
...
~~~lua
greg.hook.after("greg.NETWORK.LACPGroupRemoved", function(ctx)
...
gregBus.on("greg.NETWORK.LACPGroupRemoved", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[NETWORK] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.NETWORK.LACPGroupRemoved` as the canonical routing key end-to-end.
