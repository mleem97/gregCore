...
sidebar_label: greg.NETWORK.LACPGroupsSet
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.NETWORK.LACPGroupsSet", |payload| {
...
~~~lua
greg.hook.after("greg.NETWORK.LACPGroupsSet", function(ctx)
...
gregBus.on("greg.NETWORK.LACPGroupsSet", (payload) => {
