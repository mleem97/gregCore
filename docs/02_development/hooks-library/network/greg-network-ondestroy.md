...
sidebar_label: greg.NETWORK.OnDestroy
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.NETWORK.OnDestroy", |payload| {
...
~~~lua
greg.hook.after("greg.NETWORK.OnDestroy", function(ctx)
...
gregBus.on("greg.NETWORK.OnDestroy", (payload) => {
