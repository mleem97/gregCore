...
sidebar_label: greg.NETWORK.OnHoverOver
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.NETWORK.OnHoverOver", |payload| {
...
~~~lua
greg.hook.after("greg.NETWORK.OnHoverOver", function(ctx)
...
gregBus.on("greg.NETWORK.OnHoverOver", (payload) => {
