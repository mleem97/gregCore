...
sidebar_label: greg.NETWORK.PowerLightMaterialSet
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.NETWORK.PowerLightMaterialSet", |payload| {
...
~~~lua
greg.hook.after("greg.NETWORK.PowerLightMaterialSet", function(ctx)
...
gregBus.on("greg.NETWORK.PowerLightMaterialSet", (payload) => {
