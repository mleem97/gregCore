...
sidebar_label: greg.NETWORK.RegisterCableConnection
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.NETWORK.RegisterCableConnection", |payload| {
...
~~~lua
greg.on("greg.NETWORK.RegisterCableConnection", function(payload)
...
~~~lua
greg.hook.after("greg.NETWORK.RegisterCableConnection", function(ctx)
...
gregBus.on("greg.NETWORK.RegisterCableConnection", (payload) => {
