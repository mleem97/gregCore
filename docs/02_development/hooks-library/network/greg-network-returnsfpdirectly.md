...
sidebar_label: greg.NETWORK.ReturnSFPDirectly
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.NETWORK.ReturnSFPDirectly", |payload| {
...
~~~lua
greg.hook.after("greg.NETWORK.ReturnSFPDirectly", function(ctx)
...
gregBus.on("greg.NETWORK.ReturnSFPDirectly", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[NETWORK] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.NETWORK.ReturnSFPDirectly` as the canonical routing key end-to-end.
