...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.NETWORK.SecondActionOnClick", |payload| {
...
~~~lua
greg.hook.after("greg.NETWORK.SecondActionOnClick", function(ctx)
...
gregBus.on("greg.NETWORK.SecondActionOnClick", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[NETWORK] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.NETWORK.SecondActionOnClick` as the canonical routing key end-to-end.
