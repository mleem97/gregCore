...
sidebar_label: greg.NETWORK.SFPFromBoxRemoved
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.NETWORK.SFPFromBoxRemoved", |payload| {
...
~~~lua
greg.hook.after("greg.NETWORK.SFPFromBoxRemoved", function(ctx)
...
gregBus.on("greg.NETWORK.SFPFromBoxRemoved", (payload) => {
