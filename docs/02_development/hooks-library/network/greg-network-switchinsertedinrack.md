...
sidebar_label: greg.NETWORK.SwitchInsertedInRack
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.NETWORK.SwitchInsertedInRack", |payload| {
...
~~~lua
greg.hook.after("greg.NETWORK.SwitchInsertedInRack", function(ctx)
...
gregBus.on("greg.NETWORK.SwitchInsertedInRack", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[NETWORK] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.NETWORK.SwitchInsertedInRack` as the canonical routing key end-to-end.
