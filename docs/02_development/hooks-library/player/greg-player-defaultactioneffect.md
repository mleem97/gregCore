...
sidebar_label: greg.PLAYER.DefaultActionEffect
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.PLAYER.DefaultActionEffect", |payload| {
...
~~~lua
greg.hook.after("greg.PLAYER.DefaultActionEffect", function(ctx)
...
gregBus.on("greg.PLAYER.DefaultActionEffect", (payload) => {
