...
sidebar_label: greg.PLAYER.GainIOPSEffect
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.PLAYER.GainIOPSEffect", |payload| {
...
~~~lua
greg.hook.after("greg.PLAYER.GainIOPSEffect", function(ctx)
...
gregBus.on("greg.PLAYER.GainIOPSEffect", (payload) => {
