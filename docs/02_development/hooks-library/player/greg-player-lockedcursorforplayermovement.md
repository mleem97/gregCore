...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.PLAYER.LockedCursorForPlayerMovement", |payload| {
...
~~~lua
greg.hook.after("greg.PLAYER.LockedCursorForPlayerMovement", function(ctx)
...
gregBus.on("greg.PLAYER.LockedCursorForPlayerMovement", (payload) => {
