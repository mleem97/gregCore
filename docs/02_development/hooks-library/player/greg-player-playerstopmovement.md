...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.PLAYER.PlayerStopMovement", |payload| {
...
~~~lua
greg.hook.after("greg.PLAYER.PlayerStopMovement", function(ctx)
...
gregBus.on("greg.PLAYER.PlayerStopMovement", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[PLAYER] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.PLAYER.PlayerStopMovement` as the canonical routing key end-to-end.
