...
sidebar_label: greg.PLAYER.Warped
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.PLAYER.Warped", |payload| {
...
~~~lua
greg.hook.after("greg.PLAYER.Warped", function(ctx)
...
gregBus.on("greg.PLAYER.Warped", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[PLAYER] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.PLAYER.Warped` as the canonical routing key end-to-end.
