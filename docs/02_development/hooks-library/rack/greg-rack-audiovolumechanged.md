...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.RACK.AudioVolumeChanged", |payload| {
...
~~~lua
greg.hook.after("greg.RACK.AudioVolumeChanged", function(ctx)
...
gregBus.on("greg.RACK.AudioVolumeChanged", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[RACK] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.RACK.AudioVolumeChanged` as the canonical routing key end-to-end.
