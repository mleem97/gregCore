...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.RACK.ButtonDisablePositionsInRack", |payload| {
...
~~~lua
greg.hook.after("greg.RACK.ButtonDisablePositionsInRack", function(ctx)
...
gregBus.on("greg.RACK.ButtonDisablePositionsInRack", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[RACK] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.RACK.ButtonDisablePositionsInRack` as the canonical routing key end-to-end.
