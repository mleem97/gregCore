...
sidebar_label: greg.RACK.ComponentInitialized
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.RACK.ComponentInitialized", |payload| {
...
~~~lua
greg.hook.after("greg.RACK.ComponentInitialized", function(ctx)
...
gregBus.on("greg.RACK.ComponentInitialized", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[RACK] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.RACK.ComponentInitialized` as the canonical routing key end-to-end.
