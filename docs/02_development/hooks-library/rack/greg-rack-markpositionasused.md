...
sidebar_label: greg.RACK.MarkPositionAsUsed
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.RACK.MarkPositionAsUsed", |payload| {
...
~~~lua
greg.hook.after("greg.RACK.MarkPositionAsUsed", function(ctx)
...
gregBus.on("greg.RACK.MarkPositionAsUsed", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[RACK] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.RACK.MarkPositionAsUsed` as the canonical routing key end-to-end.
