...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SERVER.ClearErrorSign", |payload| {
...
~~~lua
greg.on("greg.SERVER.ClearErrorSign", function(payload)
...
~~~lua
greg.hook.after("greg.SERVER.ClearErrorSign", function(ctx)
...
gregBus.on("greg.SERVER.ClearErrorSign", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SERVER] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SERVER.ClearErrorSign` as the canonical routing key end-to-end.
