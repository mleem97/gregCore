...
sidebar_label: greg.SERVER.ClearWarningSign
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SERVER.ClearWarningSign", |payload| {
...
~~~lua
greg.hook.after("greg.SERVER.ClearWarningSign", function(ctx)
...
gregBus.on("greg.SERVER.ClearWarningSign", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SERVER] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SERVER.ClearWarningSign` as the canonical routing key end-to-end.
