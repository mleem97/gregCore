...
sidebar_label: greg.SERVER.ComponentInitialized
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SERVER.ComponentInitialized", |payload| {
...
~~~lua
greg.hook.after("greg.SERVER.ComponentInitialized", function(ctx)
...
gregBus.on("greg.SERVER.ComponentInitialized", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SERVER] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SERVER.ComponentInitialized` as the canonical routing key end-to-end.
