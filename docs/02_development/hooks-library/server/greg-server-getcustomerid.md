...
sidebar_label: greg.SERVER.GetCustomerID
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SERVER.GetCustomerID", |payload| {
...
~~~lua
greg.hook.after("greg.SERVER.GetCustomerID", function(ctx)
...
gregBus.on("greg.SERVER.GetCustomerID", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SERVER] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SERVER.GetCustomerID` as the canonical routing key end-to-end.
