...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SERVER.GetNextCustomerID", |payload| {
...
~~~lua
greg.hook.after("greg.SERVER.GetNextCustomerID", function(ctx)
...
gregBus.on("greg.SERVER.GetNextCustomerID", (payload) => {
