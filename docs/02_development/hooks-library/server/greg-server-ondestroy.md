...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SERVER.OnDestroy", |payload| {
...
~~~lua
greg.hook.after("greg.SERVER.OnDestroy", function(ctx)
...
gregBus.on("greg.SERVER.OnDestroy", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SERVER] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SERVER.OnDestroy` as the canonical routing key end-to-end.
