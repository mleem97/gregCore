...
sidebar_label: greg.SERVER.OnLoadingComplete
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SERVER.OnLoadingComplete", |payload| {
...
~~~lua
greg.hook.after("greg.SERVER.OnLoadingComplete", function(ctx)
...
gregBus.on("greg.SERVER.OnLoadingComplete", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SERVER] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SERVER.OnLoadingComplete` as the canonical routing key end-to-end.
