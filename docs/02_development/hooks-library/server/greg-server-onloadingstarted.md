...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SERVER.OnLoadingStarted", |payload| {
...
~~~lua
greg.hook.after("greg.SERVER.OnLoadingStarted", function(ctx)
...
gregBus.on("greg.SERVER.OnLoadingStarted", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SERVER] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SERVER.OnLoadingStarted` as the canonical routing key end-to-end.
