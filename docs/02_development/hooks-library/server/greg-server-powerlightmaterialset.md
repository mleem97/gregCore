...
sidebar_label: greg.SERVER.PowerLightMaterialSet
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SERVER.PowerLightMaterialSet", |payload| {
...
~~~lua
greg.hook.after("greg.SERVER.PowerLightMaterialSet", function(ctx)
...
gregBus.on("greg.SERVER.PowerLightMaterialSet", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SERVER] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SERVER.PowerLightMaterialSet` as the canonical routing key end-to-end.
