...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SERVER.ServerScreenUIChanged", |payload| {
...
~~~lua
greg.hook.after("greg.SERVER.ServerScreenUIChanged", function(ctx)
...
gregBus.on("greg.SERVER.ServerScreenUIChanged", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SERVER] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SERVER.ServerScreenUIChanged` as the canonical routing key end-to-end.
