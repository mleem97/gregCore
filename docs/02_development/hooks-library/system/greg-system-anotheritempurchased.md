...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.AnotherItemPurchased", |payload| {
...
~~~lua
greg.on("greg.SYSTEM.AnotherItemPurchased", function(payload)
...
~~~lua
greg.hook.after("greg.SYSTEM.AnotherItemPurchased", function(ctx)
...
gregBus.on("greg.SYSTEM.AnotherItemPurchased", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SYSTEM] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SYSTEM.AnotherItemPurchased` as the canonical routing key end-to-end.
