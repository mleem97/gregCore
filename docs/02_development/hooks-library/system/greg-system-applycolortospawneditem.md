...
sidebar_label: greg.SYSTEM.ApplyColorToSpawnedItem
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.ApplyColorToSpawnedItem", |payload| {
...
~~~lua
greg.on("greg.SYSTEM.ApplyColorToSpawnedItem", function(payload)
...
~~~lua
greg.hook.after("greg.SYSTEM.ApplyColorToSpawnedItem", function(ctx)
...
gregBus.on("greg.SYSTEM.ApplyColorToSpawnedItem", (payload) => {
