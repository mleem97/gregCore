...
sidebar_label: greg.SYSTEM.AutoSaveEnabledSet
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.AutoSaveEnabledSet", |payload| {
...
~~~lua
greg.hook.after("greg.SYSTEM.AutoSaveEnabledSet", function(ctx)
...
gregBus.on("greg.SYSTEM.AutoSaveEnabledSet", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SYSTEM] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SYSTEM.AutoSaveEnabledSet` as the canonical routing key end-to-end.
