...
sidebar_label: greg.SYSTEM.AutoSaveIntervalSet
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.AutoSaveIntervalSet", |payload| {
...
~~~lua
greg.hook.after("greg.SYSTEM.AutoSaveIntervalSet", function(ctx)
...
gregBus.on("greg.SYSTEM.AutoSaveIntervalSet", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SYSTEM] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SYSTEM.AutoSaveIntervalSet` as the canonical routing key end-to-end.
