...
sidebar_label: greg.SYSTEM.ButtonAssetManagementScreen
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.ButtonAssetManagementScreen", |payload| {
...
~~~lua
greg.hook.after("greg.SYSTEM.ButtonAssetManagementScreen", function(ctx)
...
gregBus.on("greg.SYSTEM.ButtonAssetManagementScreen", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SYSTEM] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SYSTEM.ButtonAssetManagementScreen` as the canonical routing key end-to-end.
