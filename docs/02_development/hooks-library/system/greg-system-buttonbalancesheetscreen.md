...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.ButtonBalanceSheetScreen", |payload| {
...
~~~lua
greg.hook.after("greg.SYSTEM.ButtonBalanceSheetScreen", function(ctx)
...
gregBus.on("greg.SYSTEM.ButtonBalanceSheetScreen", (payload) => {
