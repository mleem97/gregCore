...
sidebar_label: greg.SYSTEM.ButtonCancel
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.ButtonCancel", |payload| {
...
~~~lua
greg.hook.after("greg.SYSTEM.ButtonCancel", function(ctx)
...
gregBus.on("greg.SYSTEM.ButtonCancel", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SYSTEM] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SYSTEM.ButtonCancel` as the canonical routing key end-to-end.
