...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.ButtonCancelColorPicker", |payload| {
...
~~~lua
greg.hook.after("greg.SYSTEM.ButtonCancelColorPicker", function(ctx)
...
gregBus.on("greg.SYSTEM.ButtonCancelColorPicker", (payload) => {
