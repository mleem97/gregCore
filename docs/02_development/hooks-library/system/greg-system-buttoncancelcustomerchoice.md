...
sidebar_label: greg.SYSTEM.ButtonCancelCustomerChoice
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.ButtonCancelCustomerChoice", |payload| {
...
~~~lua
greg.hook.after("greg.SYSTEM.ButtonCancelCustomerChoice", function(ctx)
...
gregBus.on("greg.SYSTEM.ButtonCancelCustomerChoice", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SYSTEM] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SYSTEM.ButtonCancelCustomerChoice` as the canonical routing key end-to-end.
