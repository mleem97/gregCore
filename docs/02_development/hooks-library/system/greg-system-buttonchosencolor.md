...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.ButtonChosenColor", |payload| {
...
~~~lua
greg.hook.after("greg.SYSTEM.ButtonChosenColor", function(ctx)
...
gregBus.on("greg.SYSTEM.ButtonChosenColor", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SYSTEM] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SYSTEM.ButtonChosenColor` as the canonical routing key end-to-end.
