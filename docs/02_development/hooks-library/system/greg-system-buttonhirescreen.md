...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.ButtonHireScreen", |payload| {
...
~~~lua
greg.hook.after("greg.SYSTEM.ButtonHireScreen", function(ctx)
...
gregBus.on("greg.SYSTEM.ButtonHireScreen", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SYSTEM] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SYSTEM.ButtonHireScreen` as the canonical routing key end-to-end.
