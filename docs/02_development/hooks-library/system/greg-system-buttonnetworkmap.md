...
sidebar_label: greg.SYSTEM.ButtonNetworkMap
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.ButtonNetworkMap", |payload| {
...
~~~lua
greg.hook.after("greg.SYSTEM.ButtonNetworkMap", function(ctx)
...
gregBus.on("greg.SYSTEM.ButtonNetworkMap", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SYSTEM] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SYSTEM.ButtonNetworkMap` as the canonical routing key end-to-end.
