...
sidebar_label: greg.SYSTEM.ButtonReturnMainScreen
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.ButtonReturnMainScreen", |payload| {
...
~~~lua
greg.hook.after("greg.SYSTEM.ButtonReturnMainScreen", function(ctx)
...
gregBus.on("greg.SYSTEM.ButtonReturnMainScreen", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SYSTEM] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SYSTEM.ButtonReturnMainScreen` as the canonical routing key end-to-end.
