...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.CartTotalChanged", |payload| {
...
~~~lua
greg.hook.after("greg.SYSTEM.CartTotalChanged", function(ctx)
...
gregBus.on("greg.SYSTEM.CartTotalChanged", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SYSTEM] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SYSTEM.CartTotalChanged` as the canonical routing key end-to-end.
