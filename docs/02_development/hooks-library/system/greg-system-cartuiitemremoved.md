...
sidebar_label: greg.SYSTEM.CartUIItemRemoved
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.CartUIItemRemoved", |payload| {
...
~~~lua
greg.hook.after("greg.SYSTEM.CartUIItemRemoved", function(ctx)
...
gregBus.on("greg.SYSTEM.CartUIItemRemoved", (payload) => {
