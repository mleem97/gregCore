...
sidebar_label: greg.SYSTEM.CleanUpShop
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.CleanUpShop", |payload| {
...
~~~lua
greg.hook.after("greg.SYSTEM.CleanUpShop", function(ctx)
...
gregBus.on("greg.SYSTEM.CleanUpShop", (payload) => {
