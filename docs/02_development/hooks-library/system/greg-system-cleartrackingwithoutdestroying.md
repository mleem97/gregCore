...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.ClearTrackingWithoutDestroying", |payload| {
...
~~~lua
greg.hook.after("greg.SYSTEM.ClearTrackingWithoutDestroying", function(ctx)
...
gregBus.on("greg.SYSTEM.ClearTrackingWithoutDestroying", (payload) => {
