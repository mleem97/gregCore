...
sidebar_label: greg.SYSTEM.CloseAnyCanvas
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.CloseAnyCanvas", |payload| {
...
~~~lua
greg.hook.after("greg.SYSTEM.CloseAnyCanvas", function(ctx)
...
gregBus.on("greg.SYSTEM.CloseAnyCanvas", (payload) => {
