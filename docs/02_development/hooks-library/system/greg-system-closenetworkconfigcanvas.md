...
sidebar_label: greg.SYSTEM.CloseNetworkConfigCanvas
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.CloseNetworkConfigCanvas", |payload| {
...
~~~lua
greg.hook.after("greg.SYSTEM.CloseNetworkConfigCanvas", function(ctx)
...
gregBus.on("greg.SYSTEM.CloseNetworkConfigCanvas", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SYSTEM] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SYSTEM.CloseNetworkConfigCanvas` as the canonical routing key end-to-end.
