...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.ComponentDisabled", |payload| {
...
~~~lua
greg.hook.after("greg.SYSTEM.ComponentDisabled", function(ctx)
...
gregBus.on("greg.SYSTEM.ComponentDisabled", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SYSTEM] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SYSTEM.ComponentDisabled` as the canonical routing key end-to-end.
