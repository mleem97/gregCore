...
sidebar_label: greg.SYSTEM.ComponentInitialized
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.ComponentInitialized", |payload| {
...
~~~lua
greg.hook.after("greg.SYSTEM.ComponentInitialized", function(ctx)
...
gregBus.on("greg.SYSTEM.ComponentInitialized", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SYSTEM] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SYSTEM.ComponentInitialized` as the canonical routing key end-to-end.
