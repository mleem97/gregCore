...
sidebar_label: greg.SYSTEM.FreeUpSpawnPoint
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.FreeUpSpawnPoint", |payload| {
...
~~~lua
greg.hook.after("greg.SYSTEM.FreeUpSpawnPoint", function(ctx)
...
gregBus.on("greg.SYSTEM.FreeUpSpawnPoint", (payload) => {
