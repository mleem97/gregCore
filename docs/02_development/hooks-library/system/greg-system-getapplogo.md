...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.GetAppLogo", |payload| {
...
~~~lua
greg.hook.after("greg.SYSTEM.GetAppLogo", function(ctx)
...
gregBus.on("greg.SYSTEM.GetAppLogo", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SYSTEM] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SYSTEM.GetAppLogo` as the canonical routing key end-to-end.
