...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.GetCableSpinnerPrefab", |payload| {
...
~~~lua
greg.hook.after("greg.SYSTEM.GetCableSpinnerPrefab", function(ctx)
...
gregBus.on("greg.SYSTEM.GetCableSpinnerPrefab", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SYSTEM] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SYSTEM.GetCableSpinnerPrefab` as the canonical routing key end-to-end.
