...
sidebar_label: greg.SYSTEM.GetCustomerItemByID
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.GetCustomerItemByID", |payload| {
...
~~~lua
greg.on("greg.SYSTEM.GetCustomerItemByID", function(payload)
...
~~~lua
greg.hook.after("greg.SYSTEM.GetCustomerItemByID", function(ctx)
...
gregBus.on("greg.SYSTEM.GetCustomerItemByID", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SYSTEM] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SYSTEM.GetCustomerItemByID` as the canonical routing key end-to-end.
