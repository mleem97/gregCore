...
sidebar_label: greg.SYSTEM.GetCustomerLogo
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.GetCustomerLogo", |payload| {
...
~~~lua
greg.hook.after("greg.SYSTEM.GetCustomerLogo", function(ctx)
...
gregBus.on("greg.SYSTEM.GetCustomerLogo", (payload) => {
