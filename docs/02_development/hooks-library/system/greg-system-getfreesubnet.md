...
sidebar_label: greg.SYSTEM.GetFreeSubnet
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.GetFreeSubnet", |payload| {
...
~~~lua
greg.on("greg.SYSTEM.GetFreeSubnet", function(payload)
...
~~~lua
greg.hook.after("greg.SYSTEM.GetFreeSubnet", function(ctx)
...
gregBus.on("greg.SYSTEM.GetFreeSubnet", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SYSTEM] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SYSTEM.GetFreeSubnet` as the canonical routing key end-to-end.
