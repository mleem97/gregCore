...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.GetNextAvailableSpawnPoint", |payload| {
...
~~~lua
greg.hook.after("greg.SYSTEM.GetNextAvailableSpawnPoint", function(ctx)
...
gregBus.on("greg.SYSTEM.GetNextAvailableSpawnPoint", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SYSTEM] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SYSTEM.GetNextAvailableSpawnPoint` as the canonical routing key end-to-end.
