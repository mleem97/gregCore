...
sidebar_label: greg.SYSTEM.GetPairedLink
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.GetPairedLink", |payload| {
...
~~~lua
greg.hook.after("greg.SYSTEM.GetPairedLink", function(ctx)
...
gregBus.on("greg.SYSTEM.GetPairedLink", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SYSTEM] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SYSTEM.GetPairedLink` as the canonical routing key end-to-end.
