...
sidebar_label: greg.SYSTEM.GetPrefabForItem
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.GetPrefabForItem", |payload| {
...
~~~lua
greg.hook.after("greg.SYSTEM.GetPrefabForItem", function(ctx)
...
gregBus.on("greg.SYSTEM.GetPrefabForItem", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SYSTEM] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SYSTEM.GetPrefabForItem` as the canonical routing key end-to-end.
