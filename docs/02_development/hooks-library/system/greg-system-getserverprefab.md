...
sidebar_label: greg.SYSTEM.GetServerPrefab
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.GetServerPrefab", |payload| {
...
~~~lua
greg.hook.after("greg.SYSTEM.GetServerPrefab", function(ctx)
...
gregBus.on("greg.SYSTEM.GetServerPrefab", (payload) => {
