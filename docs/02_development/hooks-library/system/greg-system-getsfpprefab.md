...
sidebar_label: greg.SYSTEM.GetSfpPrefab
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.GetSfpPrefab", |payload| {
...
~~~lua
greg.on("greg.SYSTEM.GetSfpPrefab", function(payload)
...
~~~lua
greg.hook.after("greg.SYSTEM.GetSfpPrefab", function(ctx)
...
gregBus.on("greg.SYSTEM.GetSfpPrefab", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SYSTEM] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SYSTEM.GetSfpPrefab` as the canonical routing key end-to-end.
