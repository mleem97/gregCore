...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.GetSwitchPrefab", |payload| {
...
~~~lua
greg.hook.after("greg.SYSTEM.GetSwitchPrefab", function(ctx)
...
gregBus.on("greg.SYSTEM.GetSwitchPrefab", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SYSTEM] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SYSTEM.GetSwitchPrefab` as the canonical routing key end-to-end.
