...
sidebar_label: greg.SYSTEM.HandleObjectives
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.HandleObjectives", |payload| {
...
~~~lua
greg.hook.after("greg.SYSTEM.HandleObjectives", function(ctx)
...
gregBus.on("greg.SYSTEM.HandleObjectives", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SYSTEM] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SYSTEM.HandleObjectives` as the canonical routing key end-to-end.
