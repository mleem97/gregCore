...
sidebar_label: greg.SYSTEM.InsertedInRack
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.InsertedInRack", |payload| {
...
~~~lua
greg.on("greg.SYSTEM.InsertedInRack", function(payload)
...
~~~lua
greg.hook.after("greg.SYSTEM.InsertedInRack", function(ctx)
...
gregBus.on("greg.SYSTEM.InsertedInRack", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SYSTEM] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SYSTEM.InsertedInRack` as the canonical routing key end-to-end.
