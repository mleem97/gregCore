...
sidebar_label: greg.SYSTEM.InteractOnClick
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.InteractOnClick", |payload| {
...
~~~lua
greg.hook.after("greg.SYSTEM.InteractOnClick", function(ctx)
...
gregBus.on("greg.SYSTEM.InteractOnClick", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SYSTEM] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SYSTEM.InteractOnClick` as the canonical routing key end-to-end.
