...
sidebar_label: greg.SYSTEM.IsAnyCableConnected
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.IsAnyCableConnected", |payload| {
...
~~~lua
greg.hook.after("greg.SYSTEM.IsAnyCableConnected", function(ctx)
...
gregBus.on("greg.SYSTEM.IsAnyCableConnected", (payload) => {
