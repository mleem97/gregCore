...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.IsCustomerSuitableForBase", |payload| {
...
~~~lua
greg.hook.after("greg.SYSTEM.IsCustomerSuitableForBase", function(ctx)
...
gregBus.on("greg.SYSTEM.IsCustomerSuitableForBase", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SYSTEM] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SYSTEM.IsCustomerSuitableForBase` as the canonical routing key end-to-end.
