...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.NewCartItemSpawned", |payload| {
...
~~~lua
greg.on("greg.SYSTEM.NewCartItemSpawned", function(payload)
...
~~~lua
greg.hook.after("greg.SYSTEM.NewCartItemSpawned", function(ctx)
...
gregBus.on("greg.SYSTEM.NewCartItemSpawned", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SYSTEM] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SYSTEM.NewCartItemSpawned` as the canonical routing key end-to-end.
