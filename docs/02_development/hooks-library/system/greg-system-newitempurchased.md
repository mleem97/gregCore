...
sidebar_label: greg.SYSTEM.NewItemPurchased
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.NewItemPurchased", |payload| {
...
~~~lua
greg.on("greg.SYSTEM.NewItemPurchased", function(payload)
...
~~~lua
greg.hook.after("greg.SYSTEM.NewItemPurchased", function(ctx)
...
gregBus.on("greg.SYSTEM.NewItemPurchased", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SYSTEM] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SYSTEM.NewItemPurchased` as the canonical routing key end-to-end.
