...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.OnApplicationQuit", |payload| {
...
~~~lua
greg.hook.after("greg.SYSTEM.OnApplicationQuit", function(ctx)
...
gregBus.on("greg.SYSTEM.OnApplicationQuit", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SYSTEM] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SYSTEM.OnApplicationQuit` as the canonical routing key end-to-end.
