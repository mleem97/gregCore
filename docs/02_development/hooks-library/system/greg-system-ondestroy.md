...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.OnDestroy", |payload| {
...
~~~lua
greg.hook.after("greg.SYSTEM.OnDestroy", function(ctx)
...
gregBus.on("greg.SYSTEM.OnDestroy", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SYSTEM] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SYSTEM.OnDestroy` as the canonical routing key end-to-end.
