...
sidebar_label: greg.SYSTEM.OnHoverOver
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.OnHoverOver", |payload| {
...
~~~lua
greg.hook.after("greg.SYSTEM.OnHoverOver", function(ctx)
...
gregBus.on("greg.SYSTEM.OnHoverOver", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SYSTEM] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SYSTEM.OnHoverOver` as the canonical routing key end-to-end.
