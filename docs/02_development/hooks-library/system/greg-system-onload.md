...
sidebar_label: greg.SYSTEM.OnLoad
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.OnLoad", |payload| {
...
~~~lua
greg.hook.after("greg.SYSTEM.OnLoad", function(ctx)
...
gregBus.on("greg.SYSTEM.OnLoad", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SYSTEM] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SYSTEM.OnLoad` as the canonical routing key end-to-end.
