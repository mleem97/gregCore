...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.OpenColorPicker", |payload| {
...
~~~lua
greg.on("greg.SYSTEM.OpenColorPicker", function(payload)
...
~~~lua
greg.hook.after("greg.SYSTEM.OpenColorPicker", function(ctx)
...
gregBus.on("greg.SYSTEM.OpenColorPicker", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SYSTEM] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SYSTEM.OpenColorPicker` as the canonical routing key end-to-end.
