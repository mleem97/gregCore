...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.PhysicalItemSpawned", |payload| {
...
~~~lua
greg.on("greg.SYSTEM.PhysicalItemSpawned", function(payload)
...
~~~lua
greg.hook.after("greg.SYSTEM.PhysicalItemSpawned", function(ctx)
...
gregBus.on("greg.SYSTEM.PhysicalItemSpawned", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SYSTEM] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SYSTEM.PhysicalItemSpawned` as the canonical routing key end-to-end.
