...
sidebar_label: greg.SYSTEM.ResetTrolleyPosition
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.ResetTrolleyPosition", |payload| {
...
~~~lua
greg.hook.after("greg.SYSTEM.ResetTrolleyPosition", function(ctx)
...
gregBus.on("greg.SYSTEM.ResetTrolleyPosition", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SYSTEM] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SYSTEM.ResetTrolleyPosition` as the canonical routing key end-to-end.
