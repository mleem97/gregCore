...
sidebar_label: greg.SYSTEM.ReturnSubnet
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.ReturnSubnet", |payload| {
...
~~~lua
greg.hook.after("greg.SYSTEM.ReturnSubnet", function(ctx)
...
gregBus.on("greg.SYSTEM.ReturnSubnet", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SYSTEM] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SYSTEM.ReturnSubnet` as the canonical routing key end-to-end.
