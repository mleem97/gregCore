...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.ReturnSwitchNameFromType", |payload| {
...
~~~lua
greg.hook.after("greg.SYSTEM.ReturnSwitchNameFromType", function(ctx)
...
gregBus.on("greg.SYSTEM.ReturnSwitchNameFromType", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SYSTEM] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SYSTEM.ReturnSwitchNameFromType` as the canonical routing key end-to-end.
