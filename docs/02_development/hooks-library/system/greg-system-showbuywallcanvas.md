...
sidebar_label: greg.SYSTEM.ShowBuyWallCanvas
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.ShowBuyWallCanvas", |payload| {
...
~~~lua
greg.hook.after("greg.SYSTEM.ShowBuyWallCanvas", function(ctx)
...
gregBus.on("greg.SYSTEM.ShowBuyWallCanvas", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SYSTEM] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SYSTEM.ShowBuyWallCanvas` as the canonical routing key end-to-end.
