...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.ShowCustomerCardsCanvas", |payload| {
...
~~~lua
greg.hook.after("greg.SYSTEM.ShowCustomerCardsCanvas", function(ctx)
...
gregBus.on("greg.SYSTEM.ShowCustomerCardsCanvas", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SYSTEM] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SYSTEM.ShowCustomerCardsCanvas` as the canonical routing key end-to-end.
