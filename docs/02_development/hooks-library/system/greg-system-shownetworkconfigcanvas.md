...
sidebar_label: greg.SYSTEM.ShowNetworkConfigCanvas
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.ShowNetworkConfigCanvas", |payload| {
...
~~~lua
greg.hook.after("greg.SYSTEM.ShowNetworkConfigCanvas", function(ctx)
...
gregBus.on("greg.SYSTEM.ShowNetworkConfigCanvas", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SYSTEM] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SYSTEM.ShowNetworkConfigCanvas` as the canonical routing key end-to-end.
