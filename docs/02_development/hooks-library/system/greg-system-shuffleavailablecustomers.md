...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.ShuffleAvailableCustomers", |payload| {
...
~~~lua
greg.hook.after("greg.SYSTEM.ShuffleAvailableCustomers", function(ctx)
...
gregBus.on("greg.SYSTEM.ShuffleAvailableCustomers", (payload) => {
