...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.ShuffleAvailableSubnets", |payload| {
...
~~~lua
greg.hook.after("greg.SYSTEM.ShuffleAvailableSubnets", function(ctx)
...
gregBus.on("greg.SYSTEM.ShuffleAvailableSubnets", (payload) => {
