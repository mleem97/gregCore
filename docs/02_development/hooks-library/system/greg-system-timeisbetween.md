...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.TimeIsBetween", |payload| {
...
~~~lua
greg.hook.after("greg.SYSTEM.TimeIsBetween", function(ctx)
...
gregBus.on("greg.SYSTEM.TimeIsBetween", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SYSTEM] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SYSTEM.TimeIsBetween` as the canonical routing key end-to-end.
