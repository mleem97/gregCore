...
sidebar_label: greg.SYSTEM.TrolleyPositionLoaded
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.TrolleyPositionLoaded", |payload| {
...
~~~lua
greg.hook.after("greg.SYSTEM.TrolleyPositionLoaded", function(ctx)
...
gregBus.on("greg.SYSTEM.TrolleyPositionLoaded", (payload) => {
