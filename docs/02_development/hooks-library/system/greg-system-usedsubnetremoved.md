...
sidebar_label: greg.SYSTEM.UsedSubnetRemoved
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.SYSTEM.UsedSubnetRemoved", |payload| {
...
~~~lua
greg.hook.after("greg.SYSTEM.UsedSubnetRemoved", function(ctx)
...
gregBus.on("greg.SYSTEM.UsedSubnetRemoved", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[SYSTEM] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.SYSTEM.UsedSubnetRemoved` as the canonical routing key end-to-end.
