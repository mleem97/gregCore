...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.UI.ClearRows", |payload| {
...
~~~lua
greg.hook.after("greg.UI.ClearRows", function(ctx)
...
gregBus.on("greg.UI.ClearRows", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[UI] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.UI.ClearRows` as the canonical routing key end-to-end.
