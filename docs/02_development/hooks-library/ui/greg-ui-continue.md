...
sidebar_label: greg.UI.Continue
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.UI.Continue", |payload| {
...
~~~lua
greg.hook.after("greg.UI.Continue", function(ctx)
...
gregBus.on("greg.UI.Continue", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[UI] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.UI.Continue` as the canonical routing key end-to-end.
