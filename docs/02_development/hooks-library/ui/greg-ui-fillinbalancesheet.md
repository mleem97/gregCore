...
sidebar_label: greg.UI.FillInBalanceSheet
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.UI.FillInBalanceSheet", |payload| {
...
~~~lua
greg.hook.after("greg.UI.FillInBalanceSheet", function(ctx)
...
gregBus.on("greg.UI.FillInBalanceSheet", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[UI] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.UI.FillInBalanceSheet` as the canonical routing key end-to-end.
