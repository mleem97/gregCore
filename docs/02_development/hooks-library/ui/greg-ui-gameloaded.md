...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.UI.GameLoaded", |payload| {
...
~~~lua
greg.on("greg.UI.GameLoaded", function(payload)
...
~~~lua
greg.hook.after("greg.UI.GameLoaded", function(ctx)
...
gregBus.on("greg.UI.GameLoaded", (payload) => {
