...
sidebar_label: greg.UI.GetLatestSnapshot
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.UI.GetLatestSnapshot", |payload| {
...
~~~lua
greg.hook.after("greg.UI.GetLatestSnapshot", function(ctx)
...
gregBus.on("greg.UI.GetLatestSnapshot", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[UI] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.UI.GetLatestSnapshot` as the canonical routing key end-to-end.
