...
sidebar_label: greg.UI.HeaderRowAdded
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.UI.HeaderRowAdded", |payload| {
...
~~~lua
greg.hook.after("greg.UI.HeaderRowAdded", function(ctx)
...
gregBus.on("greg.UI.HeaderRowAdded", (payload) => {
