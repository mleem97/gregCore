...
sidebar_label: greg.UI.InstantiateRow
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.UI.InstantiateRow", |payload| {
...
~~~lua
greg.hook.after("greg.UI.InstantiateRow", function(ctx)
...
gregBus.on("greg.UI.InstantiateRow", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[UI] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.UI.InstantiateRow` as the canonical routing key end-to-end.
