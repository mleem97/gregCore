...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.UI.RestoreRecord", |payload| {
...
~~~lua
greg.hook.after("greg.UI.RestoreRecord", function(ctx)
...
gregBus.on("greg.UI.RestoreRecord", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[UI] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.UI.RestoreRecord` as the canonical routing key end-to-end.
