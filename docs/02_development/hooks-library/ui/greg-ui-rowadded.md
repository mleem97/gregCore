...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.UI.RowAdded", |payload| {
...
~~~lua
greg.on("greg.UI.RowAdded", function(payload)
...
~~~lua
greg.hook.after("greg.UI.RowAdded", function(ctx)
...
gregBus.on("greg.UI.RowAdded", (payload) => {
  const method = payload?.method ?? 'unknown';
  console.log("[UI] method=" + method);
});
~~~

When using RPC/WebSocket transport, keep `greg.UI.RowAdded` as the canonical routing key end-to-end.
