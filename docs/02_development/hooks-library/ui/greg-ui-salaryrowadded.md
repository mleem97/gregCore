...
sidebar_label: greg.UI.SalaryRowAdded
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.UI.SalaryRowAdded", |payload| {
...
~~~lua
greg.hook.after("greg.UI.SalaryRowAdded", function(ctx)
...
gregBus.on("greg.UI.SalaryRowAdded", (payload) => {
