...
sidebar_label: greg.UI.SectionTitleAdded
...
fn register_hooks(bus: &mut EventBus) {
    bus.on("greg.UI.SectionTitleAdded", |payload| {
...
~~~lua
greg.hook.after("greg.UI.SectionTitleAdded", function(ctx)
...
gregBus.on("greg.UI.SectionTitleAdded", (payload) => {
