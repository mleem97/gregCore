...
/// <see cref="UiExtensionBootstrap"/> from <see cref="Core"/>; optional greg.Plugin.* assemblies may still
...
public static class UiExtensionBridge
{
        ...
        foreach (Delegate handler in handlers.GetInvocationList())
        {
            ...
            catch (Exception ex)
            {
                CrashLog.LogException(
                    $"UiExtensionBridge.AfterTryModernize.{handler.Method.DeclaringType?.Name ?? "?"}.{handler.Method.Name}",
                    ex);
            }
        }
    }
    ...
    private static void SafeInvokeAfterUiSceneLoaded(string sceneName)
    {
        ...
        foreach (Delegate handler in handlers.GetInvocationList())
        {
            ...
            {
                CrashLog.LogException(
                    $"UiExtensionBridge.AfterUiSceneLoaded.{handler.Method.DeclaringType?.Name ?? "?"}.{handler.Method.Name}",
                    ex);
            }
        }
    }
    ...
    private static void SafeInvokeAfterDrawGui()
    {
        ...
        foreach (Delegate handler in handlers.GetInvocationList())
        {
            ...
            {
                CrashLog.LogException(
                    $"UiExtensionBridge.AfterDrawGui.{handler.Method.DeclaringType?.Name ?? "?"}.{handler.Method.Name}",
                    ex);
            }
        }
    }
    ...
    private static void SafeInvokeAfterSettingsOpened(MainMenu mainMenu)
    {
        ...
        foreach (Delegate handler in handlers.GetInvocationList())
        {
            ...
            {
                CrashLog.LogException(
                    $"UiExtensionBridge.AfterSettingsOpened.{handler.Method.DeclaringType?.Name ?? "?"}.{handler.Method.Name}",
                    ex);
            }
        }
    }
