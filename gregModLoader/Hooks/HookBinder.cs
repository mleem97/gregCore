...
public static class HookBinder
{
    ...
    /// <summary>
    /// Registers a pre-patch style handler for a canonical greg hook alias.
    /// </summary>
    /// <param name="gregHookName">greg alias such as <c>greg.Server.OnServerStart</c>.</param>
    /// <param name="handler">Handler delegate to invoke.</param>
    public static void OnBefore(string gregHookName, Action<HookContext> handler)
    {
        RegisterHandler(BeforeHandlers, gregHookName, handler);
    }

    /// <summary>
    /// Registers a post-patch style handler for a canonical greg hook alias.
    /// </summary>
    /// <param name="gregHookName">greg alias such as <c>greg.Server.OnServerStart</c>.</param>
    /// <param name="handler">Handler delegate to invoke.</param>
    public static void OnAfter(string gregHookName, Action<HookContext> handler)
    {
        RegisterHandler(AfterHandlers, gregHookName, handler);
    }

    /// <summary>
    /// Unregisters all before and after handlers for the provided hook alias.
    /// </summary>
    /// <param name="gregHookName">greg hook alias.</param>
    public static void Unregister(string gregHookName)
    {
        if (string.IsNullOrWhiteSpace(gregHookName))
            return;

        lock (SyncRoot)
        {
            BeforeHandlers.Remove(gregHookName);
            AfterHandlers.Remove(gregHookName);
        }
    }

    /// <summary>
    /// Loads raw-to-greg alias mappings from a hook dump file.
    /// </summary>
    ...
    /// <summary>
    /// Resolves an greg alias from a reflected method.
    /// </summary>
    /// <param name="method">Method to resolve.</param>
    /// <returns>Canonical greg alias.</returns>
    public static string ResolveAlias(MethodBase method)
    {
        if (method == null)
            return "greg.Misc.OnUnknown";
    ...
    private static void RegisterHandler(Dictionary<string, List<Action<HookContext>>> map, string gregHookName, Action<HookContext> handler)
    {
        if (string.IsNullOrWhiteSpace(gregHookName))
            throw new ArgumentException("greg hook name must not be empty.", nameof(gregHookName));
        ...
        lock (SyncRoot)
        {
            if (!map.TryGetValue(gregHookName, out List<Action<HookContext>> handlers))
            {
                handlers = new List<Action<HookContext>>();
                map[gregHookName] = handlers;
            }
