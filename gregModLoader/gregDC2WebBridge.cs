...
public static class DC2WebBridge
{
    ...
    static DC2WebBridge()
    {
        ...
        RegisterBundle("MainMenu", new Dc2WebBundle
        {
            ...
            Sources =
            {
                new Dc2WebSource { Type = Dc2WebSourceType.Html, Content = "<div><h1>gregCore gregMain Menu</h1><p>Unified dashboard for gameplay settings, multiplayer and mods.</p></div>" },
    ...
    private static bool TryInvokeMainMenuAction(GameObject root, string action)
    {
        ...
        if (bestMatch == null)
        {
            CrashLog.Log($"DC2WebBridge: no gregMain menu target found for action '{action}'.");
            return false;
        }
        ...
        try
        {
            ...
            CrashLog.Log($"DC2WebBridge: invoked gregMain menu action '{action}' via '{GetButtonCaption(bestMatch)}'.");
            return true;
        }
        catch (Exception ex)
        {
