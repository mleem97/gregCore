...
using MelonLoader;
using gregModLoader.LanguageBridges;
...
public class gregFfiBridge : IDisposable
{
    ...
    private readonly gregGameApiManager _apiManager;
    ...
    [UnmanagedFunctionPointer(CallingConvention.Cdecl)]
    private delegate void ModOnGuiDelegate();
    ...
    private class RustMod
    {
        ...
        public ModOnGuiDelegate OnGui;
        public ModShutdownDelegate Shutdown;
        public ModOnEventDelegate OnEvent;
    }

    public gregFfiBridge(MelonLogger.Instance logger, string modsPath)
    {
        _logger = logger;
        _modsPath = modsPath;
        _apiManager = new gregGameApiManager(logger);
    }
    ...
    private void LoadMod(string modPath)
    {
        ...
        {
            _logger.Warning($"  '{fileName}' is loaded as native adapter module. Embedded Lua/Python host is not implemented in gregCoreLoader.");
        }
        ...
        CrashLog.Log($"LoadMod: resolving optional export 'mod_on_gui' for '{mod.Name}'");
        var onGuiPtr = GetProcAddress(handle, "mod_on_gui");
        if (onGuiPtr != IntPtr.Zero)
            mod.OnGui = Marshal.GetDelegateForFunctionPointer<ModOnGuiDelegate>(onGuiPtr);
    ...
    public void OnUpdate(float deltaTime)
    {
        ...
        {
            CrashLog.LogException("gregFfiBridge.OnUpdate outer", ex);
        }
    }
    ...
    public void OnFixedUpdate(float deltaTime)
    {
        ...
        {
            CrashLog.LogException("gregFfiBridge.OnFixedUpdate outer", ex);
        }
    }
    ...
    public void OnGui()
    {
        try
        {
            foreach (var mod in _loadedMods)
            {
                ...
                try { mod.OnGui?.Invoke(); }
                catch (Exception ex)
                {
                    _logger.Error($"[{mod.Name}] mod_on_gui crashed: {ex.Message}");
                    CrashLog.LogException($"[{mod.Name}] mod_on_gui", ex);
                }
            }
        }
        catch (Exception ex)
        {
            CrashLog.LogException("gregFfiBridge.OnGui outer", ex);
        }
    }
    ...
    public void SetModEnabled(string modId, bool enabled)
    {
        ...
        gregModActivationService.SetEnabled(modId, enabled);
    }
    ...
    {
        return gregModActivationService.IsEnabled(mod.Id, true);
    }
