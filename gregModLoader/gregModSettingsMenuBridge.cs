...
public static class ModSettingsMenuBridge
{
    ...
    public static void OnSettingsOpened(MainMenu mainMenu)
    {
        ...
        var bridge = gregCoreLoader.Multiplayer;
        if (bridge != null)
        {
    ...
    public static void DrawGUI()
    {
        ...
        GUI.Box(_hubRect, "gregCore gregMain Menu Hub", _windowStyle);
        DrawHubWindow();
    }
    ...
    private static void DrawHubWindow()
    {
        ...
        GUI.Label(new Rect(x, y, w, 28f), "Modernes gregCoreLoader UI (MainMenu Replace)", _titleStyle);
        y += 28f;
    ...
    {
        GUI.Label(new Rect(x, y, w, 24f), "gregCoreLoader Hub Übersicht", _titleStyle);
        y += 30f;
    ...
    private static void DrawUiSection(float x, ref float y, float w)
    {
        ...
        bool webBridgeEnabled = gregUiExtensionBridge.GetWebBridgeEnabled();
        webBridgeEnabled = GUI.Toggle(new Rect(x, y, w, 24f), webBridgeEnabled, "DC2WEB Bridge aktiv", _labelStyle);
        gregUiExtensionBridge.SetWebBridgeEnabled(webBridgeEnabled);
        y += 28f;

        gregUiModernizer.Enabled = GUI.Toggle(new Rect(x, y, w, 24f), gregUiModernizer.Enabled, "Unity UI Modernizer aktiv", _labelStyle);
        y += 28f;
        ...
        {
            gregUiExtensionBridge.SetWebProfileReplaceMode("MainMenu", _replaceMainMenuWithWeb);
            if (_mainMenu != null)
            {
                gregUiExtensionBridge.ResetWebAppliedState(_mainMenu.gameObject);
                gregUiExtensionBridge.TryApplyOrReplace(_mainMenu.gameObject, "MainMenu");
            }
        }
    }
