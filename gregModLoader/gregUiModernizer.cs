...
/// </summary>
public static class gregUiModernizer
{
    ...
    public static void TryModernize(GameObject root, string sourceTag)
    {
        ...
        try
        {
            ...
            CrashLog.Log($"gregUiModernizer: applied modern style to '{root.name}' via {sourceTag}");
        }
        catch (Exception ex)
        {
            CrashLog.LogException($"gregUiModernizer.TryModernize({sourceTag})", ex);
        }
    }
