...
[assembly: MelonInfo(typeof(greg.Plugin.WebUIBridge.gregMain), "greg.Plugin.WebUIBridge", gregReleaseVersion.Current, "mleem97")]
...
public sealed class gregMain : gregPluginBase
{
    ...
    public override void OnFrameworkReady()
    {
        ...
        MelonLogger.Msg(
            "[greg.Plugin.WebUIBridge] Subscribed to gregUiExtensionBridge hooks; web replacement remains in gregCore.");
    }
    ...
    private static void OnAfterSettingsOpened(MainMenu mainMenu)
    {
        try
        {
            bool web = gregUiExtensionBridge.GetWebBridgeEnabled();
            MelonLogger.Msg(
