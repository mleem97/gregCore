...
public static class NPCModelReplacer
{
    ...
    {
        LoadedPlayerModel model = GregModelRegistry.GetLoadedModel(modelName);
        if (model == null)
            return;
        ...
        if (npcAnimator != null && modelAnimator != null && modelAnimator.avatar != null && !modelAnimator.avatar.isHuman)
            MelonLogger.Warning($"greg.PlayerModels: NPC '{npcId}' model '{modelName}' has non-humanoid avatar; compatibility may be limited.");

        MelonLogger.Msg($"greg.PlayerModels: applied '{modelName}' to NPC '{npc.name}'.");
    }
