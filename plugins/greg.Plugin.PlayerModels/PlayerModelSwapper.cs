...
public static class PlayerModelSwapper
{
    ...
    public static void RefreshModel(string playerId)
    {
        ...
        LoadedPlayerModel loadedModel = GregModelRegistry.GetLoadedModel(modelName);
        if (loadedModel == null)
        {
            MelonLogger.Warning($"greg.PlayerModels: assigned model '{modelName}' for '{playerId}' not loaded.");
            return;
        }
        ...
        if (target == null)
        {
            MelonLogger.Warning($"greg.PlayerModels: player '{playerId}' not found for model refresh.");
            return;
        }
    ...
    private static void ApplyModel(GameObject target, LoadedPlayerModel model)
    {
        ...
        {
            MelonLogger.Warning($"greg.PlayerModels: missing renderer while applying '{model.ModelName}' to '{target.name}'.");
            return;
        }
        ...
        MelonLogger.Msg($"greg.PlayerModels: applied '{model.ModelName}' to player '{target.name}'.");
    }
}
