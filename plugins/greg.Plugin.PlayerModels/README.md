...
`greg.Plugin.PlayerModels` adds runtime model loading and replacement for players and NPCs.
...
```csharp
greg.PlayerModels.API.LoadModel("MyCustomPlayer");
greg.PlayerModels.API.AssignModelToPlayer("76561198000000000", "MyCustomPlayer");
greg.PlayerModels.API.RefreshModel("76561198000000000");
```

## API Surface

- `greg.PlayerModels.API.LoadModel(string modelName)`
- `greg.PlayerModels.API.AssignModelToPlayer(string playerId, string modelName)`
- `greg.PlayerModels.API.SetLocalPlayerModel(string modelName)`
- `greg.PlayerModels.API.RefreshModel(string playerId)`
- `greg.PlayerModels.API.ReplaceNPCModel(string npcId, string modelName, bool persistent = true)`
- `greg.PlayerModels.API.RevertNPC(string npcId)`
...
```csharp
HookBinder.OnAfter("greg.Server.OnPowerButton", context =>
{
...
- Multiplayer sync hooks are optional and gated by `GREG_MULTIPLAYER`.
