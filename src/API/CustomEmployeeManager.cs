...
public static class CustomEmployeeManager
{
    ...
    public static int Register(string id, string name, string description, float salary, float reputation, bool requiresConfirmation = false)
    {
        ...
        greg.Logging.GregLogger.Msg($"[CustomEmployee] Registered: {name} (id={id}, salary={salary}/h, rep={reputation})");
    ...
    public static int Hire(string id)
    {
        ...
        if (playerRep < entry.RequiredReputation)
        {
            ...
            greg.Logging.GregLogger.Warn($"[CustomEmployee] Cannot hire {entry.Name}: need reputation {entry.RequiredReputation} (you have {playerRep:F0})");
            return -1;
        }

        entry.IsHired = true;
        CrashLog.Log($"CustomEmployee hired: {id} ({entry.Name})");
        greg.Logging.GregLogger.Msg($"[CustomEmployee] Hired: {entry.Name}");
    ...
    public static int Fire(string id)
    {
        ...
        try
        {
            greg.Logging.GregLogger.Msg($"[CustomEmployee] Fired: {entry.Name}");
        }
...
#pragma warning restore CS0414
    ...
    private static void SetPortrait(Transform card, string employeeId)
    {
        try
        {
            ...
            // SECURITY: Prevent path traversal by validating the employeeId before filesystem access.
            if (employeeId.IndexOfAny(Path.GetInvalidFileNameChars()) >= 0 ||
                employeeId.Contains("..") ||
                employeeId.Contains(Path.DirectorySeparatorChar.ToString()) ||
                employeeId.Contains(Path.AltDirectorySeparatorChar.ToString()))
            {
                CrashLog.Log($"Security: Path traversal attempt blocked for employee portrait '{employeeId}'");
                return;
            }
