...
public class LuaHotReload : IDisposable
{
    ...
    private string? FindModForFile(string filePath)
    {
        ...
        string rootPathSlash = rootPath;
        if (!rootPathSlash.EndsWith(Path.DirectorySeparatorChar.ToString()))
            rootPathSlash += Path.DirectorySeparatorChar;

        if (!fullPath.StartsWith(rootPathSlash, StringComparison.OrdinalIgnoreCase) &&
