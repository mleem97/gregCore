...
public class LuaHotReload : IDisposable
{
    ...
    private string? FindModForFile(string filePath)
    {
        ...
        string rootWithSlash = rootPath.EndsWith(Path.DirectorySeparatorChar.ToString()) ? rootPath : rootPath + Path.DirectorySeparatorChar;

        if (!fullPath.Equals(rootPath, StringComparison.OrdinalIgnoreCase) && !fullPath.StartsWith(rootWithSlash, StringComparison.OrdinalIgnoreCase))
            return null;
