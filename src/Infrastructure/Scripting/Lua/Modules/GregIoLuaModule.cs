...
public static class GregIoLuaModule
{
    ...
    private static string ResolveSafe(string dataDir, string relativePath)
    {
        ...
        if (!dataDirWithSep.EndsWith(Path.DirectorySeparatorChar.ToString()) && !dataDirWithSep.EndsWith(Path.AltDirectorySeparatorChar.ToString()))
            dataDirWithSep += Path.DirectorySeparatorChar;

        bool isAllowed = fullPath.StartsWith(dataDirWithSep, StringComparison.OrdinalIgnoreCase) ||
