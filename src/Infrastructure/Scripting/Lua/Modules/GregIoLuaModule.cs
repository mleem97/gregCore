...
public static class GregIoLuaModule
{
    ...
    private static string ResolveSafe(string dataDir, string relativePath)
    {
        ...
        string prefix = dataDirFull.EndsWith(Path.DirectorySeparatorChar.ToString())
            ? dataDirFull
            : dataDirFull + Path.DirectorySeparatorChar;

        if (!(fullPath.Equals(dataDirFull, StringComparison.OrdinalIgnoreCase) || fullPath.StartsWith(prefix, StringComparison.OrdinalIgnoreCase)))
