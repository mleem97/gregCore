/// <file-summary>
/// Layer:       API (Legacy)
/// Purpose:     Stable API facade for all mod languages (Lua, Rust, Go, JS, Python).
/// Maintainer:  All methods read/write real IL2CPP game objects.
///               Errors are caught and default values are returned.
///               Pointer checks prevent ObjectCollectedException.
/// </file-summary>
...
{
    /// <summary>
    /// Legacy GregAPI – stable, static facade for all scripting bridges.
    /// Each method is wrapped in try-catch, checks pointers, and returns a safe default on error.
    /// </summary>
    public static class GregAPI
    {
        private static IGregLogger _logger = null!;

        // Set by the main composition: mirrors Log() into the DevConsole.
        // Without wiring, Log() falls back to the MelonLogger.
            ...
            IGregPersistenceService? persistence)
        {
            ...
            Persistence = persistence ?? throw new ArgumentNullException(nameof(persistence),
                "Main composition must provide the persistence service.");
            UI = new GregUIController(_logger);
            UI.Initialize();
            _logger.Info("GregAPI initialized successfully.");
        }

        public static GregUIController UI { get; set; } = null!;
        public static GregEventBus EventBus { get; set; } = null!;
        public static IGregPersistenceService Persistence { get; set; } = null!;
