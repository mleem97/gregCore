...
/// Purpose:     Win32 FFI implementation for native mods.
/// Maintainer:  Encapsulates LoadLibrary, GetProcAddress and FreeLibrary. Thread-safe!
/// </file-summary>
        ...
        lock (_syncRoot)
        {
            ...
            try
            {
                ...
                if (hModule == IntPtr.Zero)
                {
                    ...
                    throw new GregBridgeException($"Could not load native mod {dllPath} not load. Win32Error: {error}");
                }

                _loadedModules.Add(hModule);
                _logger.Info($"Native mod {dllPath} loaded.");
            }
            catch (GregBridgeException ex)
            {
                _logger.Error($"[Win32FfiBridge] Bridge error: {ex.Message}", ex);
            }
        }
    }
