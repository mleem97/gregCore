...
internal static class Patch_UsableObject_InteractOnClick
{
    ...
    private static int FindPickupRackUid(UsableObject __instance, byte objectType)
    {
        try
        {
            // currentRackPosition/rackPositionUID live on UsableObject
            // (shared base) - the type check only ensures that
            // type and objectType match (otherwise -1 as before).
            switch (objectType)
            {
                case 0:
                case 1:
    ...
    // Dead pointers fall back to -1 via try/catch.
