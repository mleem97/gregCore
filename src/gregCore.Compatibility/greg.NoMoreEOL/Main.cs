    ...
    public class Main : MelonMod
    {
        ...
        private void HandleServersEOL()
        {
            ...
            foreach (var kvp in _networkMap.servers)
            {
                ...
                // eolTime existiert game-seitig nur noch auf konkretem Server
                // (INetworkEndpoint-Drift) - per TryCast auflösen.
                var server = srv.TryCast<Server>();
                if (server != null)
                {
                    server.eolTime = GetEOLDeviceDefaultEOL(false, srv.serverType);
                }
            }
        }
