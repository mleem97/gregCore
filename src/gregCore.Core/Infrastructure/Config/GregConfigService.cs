...
/// Purpose:     Config service based on Newtonsoft.Json.
/// Maintainer:  Supports JsonComments for mod.json and global configs.
/// </file-summary>
...
public sealed class GregConfigService : IGregConfigService
{
    ...
    public T? LoadConfig<T>(string filePath) where T : class
    {
        ...
        {
            _logger.Error($"Error loading Config {filePath}", ex);
            return null;
        }
    }
    ...
    public void SaveConfig<T>(string filePath, T config) where T : class
    {
        ...
        {
            _logger.Error($"Error saving Config {filePath}", ex);
        }
    }
}
