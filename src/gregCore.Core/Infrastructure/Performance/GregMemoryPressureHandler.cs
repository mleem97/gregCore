...
internal sealed class GregMemoryPressureHandler
{
    ...
    private DateTime _lastWarningGc = DateTime.MinValue;
    ...
    private void OnRamWarning(EventPayload p)
    {
        ...
        // Throttle: warning GC at most 1x per minute, otherwise the
        // pressure relief itself becomes an FPS problem.
        var now = DateTime.UtcNow;
        if ((now - _lastWarningGc).TotalSeconds < 60) return;
        _lastWarningGc = now;
