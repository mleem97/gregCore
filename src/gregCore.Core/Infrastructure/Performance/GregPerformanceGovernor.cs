...
public sealed class GregPerformanceGovernor : IGregPerformanceGovernor, IDisposable
{
    ...
    internal GregPerformanceGovernor(GregApiContext ctx, PerformanceProfile? profile = null)
    {
        ...
        // Default: FPS-only (Safe). Grafik nur bei explizitem UseProfile(Balanced/...),
        // sonst verschwinden Server-LODs bei QualityLevel/lodBias-Eingriff.
        _profile = profile ?? PerformanceProfile.Safe;
    ...
    {
        var switchingToFpsOnly = !profile.ApplyGraphicsSettings && _profile.ApplyGraphicsSettings;
        _profile = profile;
        _fpsLimiter.Apply(profile);
        if (switchingToFpsOnly)
            _fpsLimiter.RestoreGameGraphics();
        _throttler.UpdateProfile(profile);
        _queue.UpdateLimit(profile.MaxQueuedOperations);
        ApplyPatchSettings(profile);
    }

    internal void RestoreGameGraphics() => _fpsLimiter.RestoreGameGraphics();
