...
internal sealed class GregResourceMonitor : IDisposable
{
    ...
    private readonly RamAlertState _alerts = new RamAlertState();
    ...
    private void MeasureAndReport()
    {
        ...
        } catch (Exception ex) { _logger.Warning($"[Monitor] Measurement error: {ex.Message}"); }
    }

    private void CheckThresholds(ResourceSnapshot s)
    {
        // Private bytes instead of working set: under Proton/Wine the working set
        // stays permanently above every threshold (Wine mapping) and would trigger
        // a warning + Gen1 GC every 5s - visible FPS damage with no benefit.
        int ramMb = RamAlertEvaluator.SelectRamMetricMb(s.RamUsedMb, s.PrivateMemoryMb);
        var fire = _alerts.Evaluate(ramMb, _profile.RamWarningMb, _profile.RamCriticalMb);
        if (fire == RamAlertLevel.Critical)
            _bus.Publish("greg.performance.RamCritical", new EventPayload { OccurredAtUtc = DateTime.UtcNow, Data = new Dictionary<string, object> { ["ramMb"] = ramMb } });
        else if (fire == RamAlertLevel.Warning)
            _bus.Publish("greg.performance.RamWarning", new EventPayload { OccurredAtUtc = DateTime.UtcNow, Data = new Dictionary<string, object> { ["ramMb"] = ramMb } });
    }
