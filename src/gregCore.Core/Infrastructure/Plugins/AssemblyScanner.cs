...
/// Purpose:     Scans assemblies for mod classes via Mono.Cecil.
/// Maintainer:  Uses Mono.Cecil for static analysis. Assembly.LoadFrom would load IL2CPP interop assemblies into the process and cause TypeLoadExceptions.
/// </file-summary>
...
public sealed class AssemblyScanner : IAssemblyScanner
{
    public IReadOnlyList<PluginInfo> ScanDirectory(string path)
    {
        ...
        foreach (var file in IO.GregFileSystem.EnumerateFilesByExtension(path, ".dll"))
        {
            // Guard: niemals aus `.deactivated` laden (nur gregCore darf per
            // GregModActivationService verschieben, nie laden).
            if (IO.GregDeactivatedGuard.IsDeactivatedPath(file)) continue;
            try
            {
            ...
            {
                // Ignore, not a valid .NET assembly
            }
        }
