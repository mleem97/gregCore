...
/// Purpose:     Manages all registered mods and plugins.
/// Maintainer:  Responsible for lifecycle (Load, Initialize, Unload).
/// </file-summary>
...
public sealed class GregPluginRegistry : IGregPluginRegistry
{
    ...
    public void RegisterMod(ModMetadata metadata)
    {
        ...
        {
            _logger.Error("Mod registration failed: ModId is empty.");
            return;
        }
    ...
    public void LoadAll()
    {
        ...
        _logger.Info($"{_loadedPlugins.Count} plugins loaded.");
    }
    ...
    private void LoadRuntimeMod(PluginInfo plugin)
    {
        ...
        if (Infrastructure.IO.GregDeactivatedGuard.IsDeactivatedPath(plugin.AssemblyPath))
        {
            _logger.Warning($"Mod uebersprungen (deaktiviert): {plugin.Manifest.Id} ({plugin.AssemblyPath})");
            return;
        }
