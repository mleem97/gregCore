...
/// Zweck:        Lädt GregCore-native DLL-Plugins aus Mods/gregNative.
/// Maintainer:   Cecil-Discovery (kein Blind-Load), dann Assembly.LoadFrom pro Treffer.
/// </file-summary>
...
public sealed class GregNativePluginLoader
{
    ...
    private int LoadFile(string file, string nativeDir)
    {
        ...
        foreach (var typeName in typeNames)
        {
            try
            {
                ...
                if (instance == null)
                {
                    _logger.Warning($"{Path.GetFileName(file)}: {typeName} implementiert IGregNativePlugin nicht.");
                    continue;
                }
                if (string.IsNullOrWhiteSpace(instance.Id))
                {
                    _logger.Warning($"{Path.GetFileName(file)}: Plugin-Id ist leer — übersprungen.");
                    continue;
                }
                if (_loaded.ContainsKey(instance.Id))
                {
                    _logger.Warning($"Doppelte Native-Plugin Id '{instance.Id}' — übersprungen.");
                    continue;
                }
    ...
    private List<string> DiscoverPluginTypes(string file)
    {
        ...
        try
        {
            ...
            foreach (var type in module.Types.SelectMany(AllTypes))
            {
                ...
                catch { /* einzelnen Typ überspringen */ }
            }
        }
        catch { /* keine gültige .NET Assembly */ }
        return result;
    }
    ...
    private static void SafeDispose(Entry entry)
    {
        ...
        catch { /* Dispose darf Shutdown nie brechen */ }
    }
    ...
    private sealed class Host : IGregNativeHost
    {
        ...
        public void PublishEvent(string hookName, IReadOnlyDictionary<string, object> data)
        {
            ...
            catch { /* Events dürfen Plugins nie brechen */ }
        }
    }
}
