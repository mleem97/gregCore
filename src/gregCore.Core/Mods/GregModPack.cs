...
{
    // ── Managed DTOs (game-independent, testable) ────────────────────────────
    ...
    // Heal null lists (vanilla/workshop packs are not always complete).
    ...
    // ── Builder: DTO -> game type (best-effort per field each) ───────────────
    ...
    // ── Reader: arbitrary pack -> managed snapshot ───────────────────────────
    ...
    private static void Try(Action action)
    {
        ...
        {
            try { MelonLogger.Warning($"[gregCore][Mods] ModPack field failed: {ex.GetBaseException().Message}"); } catch { /* ignored: defensive best-effort (CONVENTIONS.md) */ }
        }
    }
}
