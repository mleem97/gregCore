...
/// Purpose:     Save DTOs for customers (CustomerBase, CustomerRecord):
///               create, fill, read, upsert by customerBaseID
///               (list: NetworkSaveData.customerBases). All best-effort.
/// </file-summary>
...
public static class GregCustomerSaves
{
    ...
    private static void Try(Action action)
    {
        ...
        {
            try { MelonLogger.Warning($"[gregCore][Net] CustomerSaves field failed: {ex.GetBaseException().Message}"); } catch { /* ignored: defensive best-effort (CONVENTIONS.md) */ }
        }
    }
}
