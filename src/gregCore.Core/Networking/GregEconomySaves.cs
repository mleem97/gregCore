...
/// Purpose:     Save DTOs for economy (MonthlySnapshot, BalanceSheet):
///               create, fill, read (nested records included).
///               Source: SaveData.balanceSheetData. All best-effort.
/// </file-summary>
...
public static class GregEconomySaves
{
    ...
    private static void Try(Action action)
    {
        ...
        {
            try { MelonLogger.Warning($"[gregCore][Net] EconomySaves field failed: {ex.GetBaseException().Message}"); } catch { /* ignored: defensive best-effort (CONVENTIONS.md) */ }
        }
    }
}
