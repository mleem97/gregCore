...
/// Purpose:     Game volume bridge (master, music, effects, racks):
///               set plus settings reload. For mods (e.g. MusicPlayer)
///               that want to respect the vanilla mixers. All best-effort.
/// </file-summary>
...
public static class GregGameSettings
{
    ...
    public static bool ReloadSettings()
    {
        ...
        {
            Warn($"LoadSettings failed: {Base(ex)}");
            return false;
        }
    }
    ...
    private static bool SetVolume(string label, float volume, Action<global::Il2Cpp.SettingsVolume, float> setter)
    {
        ...
        {
            Warn($"{label} volume failed: {Base(ex)}");
            return false;
        }
    }
    ...
    private static void Try(Action action)
    {
        ...
        {
            try { MelonLogger.Warning($"[gregCore][Net] GameSettings field failed: {ex.GetBaseException().Message}"); } catch { /* ignored: defensive best-effort (CONVENTIONS.md) */ }
        }
    }
}
