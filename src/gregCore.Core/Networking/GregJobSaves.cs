...
/// Purpose:     Save DTOs for jobs + interact objects (Technician,
///               RepairJob, InteractObject): create, fill, read;
///               upsert by uid for InteractObjects (lists:
...
public static class GregJobSaves
{
    ...
    private static void Try(Action action)
    {
        ...
        {
            try { MelonLogger.Warning($"[gregCore][Net] JobSaves field failed: {ex.GetBaseException().Message}"); } catch { /* ignored: defensive best-effort (CONVENTIONS.md) */ }
        }
    }
}
