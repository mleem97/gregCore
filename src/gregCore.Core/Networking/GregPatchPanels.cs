...
/// Zweck:        PatchPanel-Brücke: Save-DTO (PatchPanelSaveData) plus
///               Erzeugen/Fuellen/Lesen/Upsert in der spieleigenen Liste
///               (NetworkSaveData.patchPanels, Key patchPanelID) sowie
///               sichere Runtime-Helfer (Capture, InsertIntoRack, FindAll,
///               IsAnyCableConnected, ValidateRackPosition). Alles best-effort.
/// </file-summary>
...
public static class GregPatchPanels
{
    ...
    // ── Upsert / Remove (Key: patchPanelID; leere ID -> immer Anhang) ────────
    ...
    // ── Runtime-Brücke (live PatchPanel) ─────────────────────────────────────
    ...
    // Live-Panel -> DTO (RackPositionUID ist runtime-seitig nicht lesbar -> -1).
    ...
    // Vanilla-Restore-Pfad: Panel per Save-Eintrag ins Rack einsetzen.
    public static bool InsertIntoRack(global::Il2Cpp.PatchPanel panel, global::Il2Cpp.PatchPanelSaveData entry)
    {
        ...
        {
            Warn($"InsertedInRack fehlgeschlagen: {Base(ex)}");
            return false;
        }
    }
