...
/// Purpose:     Server bridge: save DTO (ServerSaveData) plus create/
///               fill/read/upsert in the game's own list
///               (NetworkSaveData.servers, key serverID) as well as safe
///               runtime helpers (Find, Capture, InsertIntoRack, SetIP, Power,
///               Customer/App, Repair, Validate). All best-effort.
/// </file-summary>
...
public static class GregServers
{
    ...
    // ── Upsert / remove (key: serverID; empty ID -> always append) ─────────────
    ...
    // Live server -> DTO (RackPositionUID is not readable runtime-side -> -1).
    ...
    // Vanilla restore path: insert server into the rack via save entry.
    public static bool InsertIntoRack(global::Il2Cpp.Server server, global::Il2Cpp.ServerSaveData entry)
    {
        ...
        {
            Warn($"ServerInsertedInRack failed: {Base(ex)}");
            return false;
        }
    }
    ...
    // Safe actions (vanilla paths, bool instead of exceptions).
    ...
    public static bool SetPower(global::Il2Cpp.Server server, bool forceState)
    {
        ...
        {
            Warn($"PowerButton failed: {Base(ex)}");
            return false;
        }
    }
    ...
    public static bool UpdateCustomer(global::Il2Cpp.Server server, int customerID)
    {
        ...
        {
            Warn($"UpdateCustomer failed: {Base(ex)}");
            return false;
        }
    }
    ...
    public static bool UpdateAppID(global::Il2Cpp.Server server, int appID)
    {
        ...
        {
            Warn($"UpdateAppID failed: {Base(ex)}");
            return false;
        }
    }
    ...
    public static bool ClearWarning(global::Il2Cpp.Server server, bool preserved)
    {
        ...
        {
            Warn($"ClearWarningSign failed: {Base(ex)}");
            return false;
        }
    }
    ...
    {
        try { MelonLogger.Warning($"[gregCore][Net] Servers: {message}"); } catch { /* ignored: defensive best-effort (CONVENTIONS.md) */ }
    }
    ...
    private static void Try(Action action)
    {
        ...
        {
            try { MelonLogger.Warning($"[gregCore][Net] Servers field failed: {ex.GetBaseException().Message}"); } catch { /* ignored: defensive best-effort (CONVENTIONS.md) */ }
        }
    }
}
