...
/// Zweck:        SetIP-Brücke (Vanilla-IP-Keypad): Instanz finden, Subnetz-
///               mathe (Maske, nutzbare IPs, erste nutzbare IP) über Vanilla
///               mit verwaltetem Fallback, sowie Show/Cancel-Passthroughs.
///               Alles best-effort.
/// </file-summary>
...
{
    // Live-Keypad finden (bevorzugt aktiv, in geladener Szene).
    ...
    // "255.255.255.0" aus 24 — Vanilla bevorzugt, verwalteter Fallback.
    ...
    // Keypad für einen Server öffnen (Vanilla-Flow).
    public static bool ShowFor(global::Il2Cpp.Server server)
    {
        ...
        {
            Warn($"ShowCanvas fehlgeschlagen: {Base(ex)}");
            return false;
        }
    }
    ...
    public static bool Cancel()
    {
        ...
        {
            Warn($"Cancel fehlgeschlagen: {Base(ex)}");
            return false;
        }
    }

    // Verwalteter Fallback (deterministisch, ohne Spiel-Instanz).
