...
public sealed record PerformanceProfile
{
    ...
    // Graphics / Quality
    // WICHTIG: Grafik-Eingriffe sind OPT-IN. Der Governor startet mit
    // ApplyGraphicsSettings=false (Safe), damit Spiel-LODs/Materialien
    // (z.B. Server in Racks via RackMount.ApplyMaterialToLODs) unangetastet bleiben.
    public bool ApplyGraphicsSettings { get; init; } = false;
    ...
    public static PerformanceProfile Safe => new PerformanceProfile();
    public static PerformanceProfile Balanced => new PerformanceProfile { ApplyGraphicsSettings = true };
