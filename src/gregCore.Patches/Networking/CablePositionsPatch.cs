/// <file-summary>
/// Layer:       GameLayer
/// Purpose:     Prefix bypass for CablePositions.CreateNewCable (hollow IL2CPP method).
/// Maintainer:  The original method always returns 0, which causes ID collisions.
///               This patch generates thread-safe unique IDs via an atomic counter.
...
public static class CablePositionsPatch
{
    ...
    public static void SetBaseId(int baseId)
    {
        ...
        try { MelonLogger.Msg($"[CablePatch] Cable ID counter set to {baseId + 1}"); }
        catch { /* logging may not be initialized in static/unit-test contexts */ }
    }
