...
public class LuaModuleLoader
{
    ...
    {
        // Guard: never load from `.deactivated` (only gregCore may move via
        // GregModActivationService, never load).
        if (global::gregCore.Infrastructure.IO.GregDeactivatedGuard.IsDeactivatedPath(fullPath))
            throw new UnauthorizedAccessException(
                $"Deactivated mod content is never loaded: {fullPath}");
