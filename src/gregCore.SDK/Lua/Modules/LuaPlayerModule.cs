...
public static class LuaPlayerModule
{
    public static void Register(Table greg, Script script, string modId)
    {
        ...
        RegisterMoney(playerTable, modId);
        RegisterProgress(playerTable, modId);
        RegisterMovement(playerTable, modId);
        greg["player"] = playerTable;
    }

    private static void RegisterPosition(Table playerTable, Script script)
    {
        // greg.player.position() -> {x, y, z}
        playerTable["position"] = (Func<Table>)(() => ReadPosition(script));
    }

    private static Table ReadPosition(Script script)
    {
        try
        {
    ...
    private static void RegisterMoney(Table playerTable, string modId)
    {
    ...
    private static void RegisterProgress(Table playerTable, string modId)
    {
    ...
    private static void RegisterMovement(Table playerTable, string modId)
    {
        // greg.player.teleport(x, y, z)
        playerTable["teleport"] = (Action<double, double, double>)((x, y, z) => TryTeleport(x, y, z, modId));
    ...
    private static void TryTeleport(double x, double y, double z, string modId)
    {
        try
        {
            var pm = Il2Cpp.PlayerManager.instance;
