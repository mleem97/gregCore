/// <file-summary>
/// Layer:       UI
/// Purpose:     Reusable mod panel (gregCore.UI kit).
///              Theming via GregUITheme, visibility + slide animation
///              + draggable header. The content (content container) is filled
///              by the mod; click delivery runs via GregUIInputSystem
///              (real callbacks) or mod-side hit routing as fallback.
///              Ticks run centrally via GregMenuRegistry.Tick.
/// </file-summary>
...
public sealed class GregPanel
{
    ...
    public void Show()
    {
        ...
        {
            MelonLogger.Warning($"[gregCore][UI] Panel-Show failed ({MenuId}): {ex.Message}");
        }
    }
    ...
    public void Hide()
    {
        ...
        {
            MelonLogger.Warning($"[gregCore][UI] Panel-Hide failed ({MenuId}): {ex.Message}");
        }
    }
    ...
    // Refill content (mod builds buttons/labels into the container).
    public void RebuildContent(Action<VisualElement> build)
    {
        ...
        {
            MelonLogger.Warning($"[gregCore][UI] Panel-Rebuild failed ({MenuId}): {ex.Message}");
        }
    }
    ...
    private void BuildChrome(string title)
    {
        ...
        if (layer == null)
        {
            MelonLogger.Error("[gregCore][UI] Panel layer not available.");
            return;
        }
