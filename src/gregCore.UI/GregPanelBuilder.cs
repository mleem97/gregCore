...
namespace gregCore.UI
{
    ...
    public sealed class GregPanelBuilder
    {
        ...
        /// <summary>
        /// Crash-safe text fields created via <see cref="AddSafeInputField"/>.
        /// The owning mod must call <c>field.PumpFrame()</c> once per frame
        /// while the field is focused ( IL2CPP has no working text input —
        /// keys are pumped manually, see <see cref="GregSafeTextField"/>).
        /// </summary>
        public IReadOnlyList<GregSafeTextField> SafeInputFields => _safeFields;

        private readonly List<GregSafeTextField> _safeFields = new();
        ...
        [Obsolete("Crashes on IL2CPP builds (stripped TextEditor) — use AddSafeInputField instead.")]
        ...
        /// <summary>
        /// IL2CPP-safe text input (Label rendering + manual keyboard pump —
        /// <see cref="GregSafeTextField"/>). The owning mod must call
        /// <c>field.PumpFrame()</c> once per frame while focused (see
        ...
        public GregPanelBuilder AddSafeInputField(string label, string currentValue, Action<string> onChanged, bool multiline = false, int maxLength = 512)
        {
            ...
            var field = new GregSafeTextField(currentValue ?? "", maxLength, multiline);
            field.InnerLabel.style.color = new Color(0.92f, 0.94f, 0.96f);
            field.InnerLabel.style.fontSize = 14;
            field.style.minHeight = multiline ? 90 : 30;
            if (onChanged != null)
            {
                field.Changed += new Action<string>(v =>
                {
                    try { onChanged(v); } catch { /* ignored: defensive best-effort (CONVENTIONS.md) */ }
                });
            }
            container.Add(field);
