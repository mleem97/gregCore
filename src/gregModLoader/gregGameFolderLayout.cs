...
/// greg plugin DLLs are deployed under <c>greg/Plugins</c>; MelonLoader still loads mods from <see cref="MelonEnvironment.ModsDirectory"/> (see docs).
/// </summary>
public static class GameFolderLayout
{
	...
	/// <summary>greg / greg extension plugins (DLLs). MelonLoader does not scan this folder — see documentation for load options.</summary>
	public static string GregPluginsDirectory => Path.Combine(MelonEnvironment.GameRootDirectory, "greg", "Plugins");
	...
	public static void EnsureGregPluginsDirectoryExists()
	{
		Directory.CreateDirectory(GregPluginsDirectory);
	}
