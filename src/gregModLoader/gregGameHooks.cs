...
public static class gregGameHooks
{
    ...
    // --- VLAN Management (v1.0.45.5+) ---

    public static int SetVlanAllowed(string switchId, int portIndex, int vlanId)
    {
        try
        {
            var nm = NetworkMap.instance;
            if (nm == null || nm.switches == null) return 0;
            if (!nm.switches.ContainsKey(switchId)) return 0;

            var sw = nm.switches[switchId];
            if (sw == null) return 0;
    ...
    public static int SetVlanDisallowed(string switchId, int portIndex, int vlanId)
    {
        try
        {
            var nm = NetworkMap.instance;
    ...
    public static int IsVlanAllowed(string switchId, int portIndex, int vlanId)
    {
        try
        {
            var nm = NetworkMap.instance;
    ...
    public static FocusHit? RaycastForward(float maxDistance)
    {
        try
        {
    ...
    // --- HUD / Unity IMGUI Helpers ---

    public static void GuiBeginPanel(string id, float x, float y, float width, float height)
    {
        try
        {
            GUI.Box(new Rect(x, y, width, height), id);
            GUILayout.BeginArea(new Rect(x + 5, y + 20, width - 10, height - 25));
        }
        catch { }
    }

    public static void GuiLabel(string text)
    {
        try
        {
    ...
    public static void GuiEndPanel()
    {
        try
        {
            GUILayout.EndArea();
