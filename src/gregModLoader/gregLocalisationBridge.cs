...
/// without modifying the gregCoreLoader i18n system.
/// 
/// Previously part of ModigAPIs, now consolidated within gregCore gregCoreLoader.
/// 
/// Use via gregApi.RegisterLocalisationResolver() / UnregisterLocalisationResolver().
/// </summary>
public static class gregLocalisationBridge
{
    ...
    public static bool TryResolve(int textId, string currentText, out string resolvedText)
    {
        ...
        foreach (var pair in resolvers)
        {
            ...
            {
                CrashLog.LogException($"gregLocalisationBridge resolver '{pair.Key}'", ex);
            }
        }
