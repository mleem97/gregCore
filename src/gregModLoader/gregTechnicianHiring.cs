...
public static class TechnicianHiring
{
        ...
        try
        {
            ...
            foreach (var def in _definitions)
            {
                try
                {
                    ...
                    gregCoreLoader.Instance?.LoggerInstance.Msg(
    ...
    public static void OnEmployeeHired(string employeeId)
    {
        try
        {
            ...
            if (tm == null)
            {
                ...
                gregCoreLoader.Instance?.LoggerInstance.Error("[TechnicianHiring] TechnicianManager not available");
                return;
            }
            ...
            if (existing == null || existing.Count == 0)
            {
                ...
                gregCoreLoader.Instance?.LoggerInstance.Error("[TechnicianHiring] No existing technicians to clone");
                return;
            }
            ...
            gregCoreLoader.Instance?.LoggerInstance.Msg($"[TechnicianHiring] Spawned technician #{newId} for '{employeeId}'");
        }
        catch (Exception ex)
        {
    ...
    public static void OnEmployeeFired(string employeeId)
    {
        try
        {
            ...
            if (_spawnedTechIds.Count == 0)
            {
                ...
                gregCoreLoader.Instance?.LoggerInstance.Warning("[TechnicianHiring] No spawned technicians to fire");
                return;
            }
            ...
            gregCoreLoader.Instance?.LoggerInstance.Msg($"[TechnicianHiring] Fired technician #{id} for '{employeeId}'");
        }
        catch (Exception ex)
        {
    ...
    public static void RestoreOnLoad()
    {
        try
        {
            ...
            if (restored > 0)
            {
                gregCoreLoader.Instance?.LoggerInstance.Msg($"[TechnicianHiring] Restored {restored} extra technician(s) from save");
            }
        }
        catch (Exception ex)
        {
