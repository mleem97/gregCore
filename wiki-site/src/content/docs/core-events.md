...
- Harmony patches derive from `SafePatch`; one task per patch class, `try/catch` inside, explicit `PatchAll` — never per-frame reflection (see [Developer Harmony IL2CPP](./developer-harmony-il2cpp/)).
...
- Callbacks run on the **main thread** (`GregMainThreadDispatcher`). Timers follow the same rule — see [Developer Timers Coroutines](./developer-timers-coroutines/).
...
- Full hook catalog by group: [Hooks Reference](./hooks-reference/)
- Timers/coroutines: [Developer Timers Coroutines](./developer-timers-coroutines/)
- Persistence events + save lifecycle: [Core Save Engine](./core-save-engine/)
- Hands-on subscribing: [Developer Events Hooks Guide](./developer-events-hooks-guide/)
