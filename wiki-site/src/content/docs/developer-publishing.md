...
- [ ] [Developer Best Practices](./developer-best-practices/) §6 diagnostics green (doctor, patcher log, contracts, version, tests, Codacy local).
...
- Never reuse shop/item IDs or GUIDs across versions (see [Developer Shop Items](./developer-shop-items/)); never rename a shipped custom event — add a new one ([Developer Events Hooks Guide](./developer-events-hooks-guide/)).
...
- Watch loader logs for `[DynamicPatcher]` failures after each **game update**, then follow [Developer Harmony IL2CPP](./developer-harmony-il2cpp/) “After a game update” (re-copy interop, regenerate `GameApi` + hooks, validate, vanilla-first test).
