---
title: "Guidebook Modelling Overview"
---
...
> Pipeline code: `GregObjImport` (validation) → `GregCustomItems` (vanilla `ModLoader` loading) → `GregModPack` (DTOs) → shop/cart/save. Lua surface: `greg.items`. Full spec tables: [Guidebook Modelling Shop Item](./guidebook-modelling-shop-item/) and [Guidebook Modelling Static Item](./guidebook-modelling-static-item/).
...
- The importer is the vanilla `Il2Cpp.ObjImporter.ImportOBJ`: pure triangle soup. Armatures, animations, shape keys, and skinning do not survive — strip them before export (see [Guidebook Modelling OBJ Blender](./guidebook-modelling-obj-blender/)).
...
- Blender (or equivalent) + this track's export checklist ([Guidebook Modelling OBJ Blender](./guidebook-modelling-obj-blender/)).
- A text editor (Lua) or .NET 6 SDK (C#) — same prerequisites as the other tracks ([Guidebook Prerequisites](./guidebook-prerequisites/)).
- Patience for one test loop: export → register → check log → look in-game → adjust scale → repeat.

## Track order

1. [Guidebook Modelling OBJ Blender](./guidebook-modelling-obj-blender/) — the OBJ contract + export checklist (read first, saves hours).
2. [Guidebook Modelling Shop Item](./guidebook-modelling-shop-item/) — buyable hardware end-to-end.
3. [Guidebook Modelling Static Item](./guidebook-modelling-static-item/) — deco/statics end-to-end.
4. [Guidebook Modelling Troubleshooting](./guidebook-modelling-troubleshooting/) — every failure mode with log line + fix.
