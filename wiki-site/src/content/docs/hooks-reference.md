...
The complete hook/event catalog — the gregCore equivalent of Oxide's hook index. Source of truth: `game_hooks.json` (1,850 methods, 22 groups) + `framework/greg_hooks.json` (canonical registry, manifest v2) + generated `docs/FrameworkAPI.md`. Usage patterns: [Developer Events Hooks Guide](./developer-events-hooks-guide/); concepts: [Core Events](./core-events/).
...
- `greg.computer` — `register_shortcut(id, label, fn_or_appid)`, `unregister_shortcut(id)`, `register_app(appId, title, on_open_fn[, on_close_fn])`, `unregister_app(appId)`, `open_app(appId)`, `close_app()`, `current_app()`, `list_shortcuts()`, `list_apps()` (see [Guidebook Computer UI](./guidebook-computer-ui/))
