...
The two JSON files are **required**: without `game_hooks.json` next to the DLL you get "No hooks found" and no events fire (see [FAQ Troubleshooting](./faq-troubleshooting/)).
...
- Save backups from before gregCore ever touched a save are kept permanently under `pre-greg/` inside `Documents/DatacenterBackups` (see [Player Mod Users Guide](./player-mod-users-guide/)).

## Next

- [Player Mod Users Guide](./player-mod-users-guide/) — daily use: Hub, HUD, settings, REPL, save hygiene.
- Something failed? [FAQ Troubleshooting](./faq-troubleshooting/) first, then Discord / GitHub issue with log excerpt.
